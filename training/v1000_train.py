...
print("=" * 75)
print("  ML OPSIDIAN v1000 - TARGET DENOISING (SELF-DISTILLATION)")
print("=" * 75)
...
# -----------------------------------------------------------------
print("\n[LOAD] Loading data...")
train_df = pd.read_csv(os.path.join(DATA_DIR, "train_v1001_gee.csv"))
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v1000 PIPELINE")
print("=" * 75)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        lgb_rmse.fit(X_tr, y_tr, eval_set=[(X_va, y_va)], callbacks=[lgb.early_stopping(150, verbose=False)])
        ...
        y_va_arr = original_target.iloc[va_idx_clean].values
        ...
        print(f"      Fold {fold+1}/5 | XGB1={xgb_m1.best_iteration:<4} CAT1={cat_m1.best_iteration_:<4} CAT2={cat_m2.best_iteration_:<4} LGB_R={lgb_rmse.best_iteration_:<4} LGB={lgb_m1.best_iteration_:<4} XGB2={xgb_m2.best_iteration:<4} | [ENS MAE={f_mae:.4f}] [{time.time() - t0:.0f}s]")
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v1000 - Target Denoising)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v1000.csv", index=False)
fold_report.to_csv("submissions/fold_report_v1000.csv", index=False)
...
submission.to_csv("submission_v1000.csv", index=False)
submission.to_csv("submissions/submission_v1000.csv", index=False)
print(f"[DONE] Saved submission_v1000.csv ({len(submission)} rows)")

np.save("oof_v1000.npy", all_oof_stacked)
np.save("submissions/oof_v1000.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v1000.npy")

# -----------------------------------------------------------------
# 8. POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v1000)")
print("=" * 75)
...
np.save("oof_v1000_optimized.npy", opt_oof)
np.save("submissions/oof_v1000_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v1000_optimized.npy")
...
submission_opt.to_csv("submission_v1000_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v1000_optimized.csv", index=False)
print(f"[DONE] Saved submission_v1000_optimized.csv ({len(submission_opt)} rows)")
