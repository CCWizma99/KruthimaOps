...
from app.weather import fetch_max_rainfall_7d, fetch_max_rainfall_7d_batched
...
def get_historical_forecast(district: str, target_date_str: str) -> Dict[str, Any]:
    """
    Run a simulation for a single district on any date (past or future).
    Uses multi-point sampling (5 points per district) to capture
    the MAXIMUM 7-day rainfall across the district boundary.

    - Past dates → Open-Meteo Archive API (observed data)
    - Future dates → Open-Meteo Forecast API (predicted data)
    """
    ...
    source = "archive" if target < today else "forecast"

    # Multi-point sampling: get MAX rainfall across 5 district points
    sum_7d = fetch_max_rainfall_7d(district, target, ref)
    ...
    logger.info(
        f"[Forecast] {district} {target_date_str}: "
        f"rain={sum_7d:.1f}mm score={score:.4f} level={level} "
        f"(multi-point max, source={source})"
    )
...
def get_historical_forecasts_batched(districts_list: list[str], target_date_str: str) -> Dict[str, Any]:
    """
    Batched version of get_historical_forecast with multi-point sampling.
    Uses 5 sample points per district and takes the MAX rainfall.
    Supports both historical and future dates.
    """
    from datetime import datetime
    target = datetime.strptime(target_date_str, "%Y-%m-%d").date()
    today = date.today()
    ...
    ref = get_district_reference()
    valid_districts = [d for d in districts_list if d in ref]
    if not valid_districts:
        return {}

    # Multi-point batched rainfall fetch (one API call per district, 5 points each)
    rainfall_map = fetch_max_rainfall_7d_batched(valid_districts, target, ref)

    results = {}
    for dist in valid_districts:
        sum_7d = rainfall_map.get(dist, 50.0)
