"""
ML Opsidian: Genesis vmalYJ - vmal + Yeo-Johnson Target Transformation
=======================================================================
Base: vmal (v703 + Custom Imputation + Adversarial Drop + 5 Seeds, LB 0.38203)

Single New Change vs. vmal:
- FOLD-AWARE Yeo-Johnson target transformation applied inside each CV fold.
  The PowerTransformer is fitted ONLY on tr_rows real targets, then applied
  to all tr_rows targets. Inverse transform is applied before stacking in
  original [0,1] space.

Why this combination is novel:
  - v64 proved YJ target transform works (LB=0.38256) but used dirty imputation
    with 'missing' category artifacts corrupting the target distribution.
  - vmal's district-mode imputation eliminates the 'missing' artifact,
    giving a cleaner target distribution with different YJ lambda parameters.
  - Conflict resolution on vmal's fold with round(3) coarser key also
    operates on the pre-transform targets, then YJ is applied.
  - This specific combination (clean imputation + conflict resolution + YJ) is untested.

Key architectural choices:
  - YJ is fitted PER FOLD on tr_rows real targets only (zero leakage from va/test).
  - Conflict resolution runs BEFORE YJ transform (operates on raw targets).
  - All OOF/test predictions are inverse-transformed back to [0,1] before stacking.
  - Stacker evaluates in original target space (no transform leakage).
  - Stable metric coefficients (vmal values) preserved throughout.
  - 5 seeds, 5-fold spatial GroupKFold, full pseudo-labeling from v703.
"""
...
# -----------------------------------------------------------------
USE_PSEUDO = True
SEEDS = [42, 2024, 7, 100, 999]

print("=" * 75)
print("  ML OPSIDIAN vmalYJ — CLEAN IMPUTATION + ADVERSARIAL DROP + YEO-JOHNSON")
print("=" * 75)
...
# -----------------------------------------------------------------
# 1.6. SEMI-SUPERVISED PSEUDO-LABELING (from raw v703)
# -----------------------------------------------------------------
...
# 2.3. Soil and Drainage Attributes
...
for df in [train_df, test_df]:
    ...
    for idx in df[mask].index:
        ...
        df.loc[idx, 'drainage_index'] = global_drainage_median if np.isnan(val) else val
...
# -----------------------------------------------------------------
# 3. FEATURE ENGINEERING (identical to vmal)
# -----------------------------------------------------------------
...
# -----------------------------------------------------------------
# 5. MODEL DEFINITIONS (identical to vmal)
# -----------------------------------------------------------------
MODEL_NAMES = [
    "XGB-MAE-1 (d7)",
    "CAT-MAE-1 (d5)",
...
N_FOLDS = 5
TARGET_RAW = "flood_risk_score"  # original target column

# The stacker evaluates in ORIGINAL [0,1] space — use real (non-pseudo) rows only
real_mask_global = train_df['is_pseudo'] == 0
original_y     = train_df.loc[real_mask_global, TARGET_RAW].values
original_df    = train_df[real_mask_global].reset_index(drop=True)
original_groups = original_df['grid_id'].values

# Global stats computed on ORIGINAL real targets (for target encoding fallbacks)
GLOBAL_MEAN   = float(original_y.mean())
GLOBAL_STD    = float(original_y.std())
GLOBAL_Q25    = float(np.percentile(original_y, 25))
GLOBAL_Q75    = float(np.percentile(original_y, 75))
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED vmalYJ PIPELINE")
print("=" * 75)

t_start_global = time.time()

# Stable metric coefficients (inherited from vmal)
...
# (operates in original [0,1] space — same as vmal)
# -----------------------------------------------------------------
...
for seed in SEEDS:
    print(f"\n==================== RUNNING SEED {seed} ====================")
    y_full = train_df[TARGET_RAW].values  # always use original target values

    oof_preds = {m: np.zeros(len(train_df)) for m in MODEL_NAMES}
    tst_preds = {m: np.zeros(len(test_df))  for m in MODEL_NAMES}

    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y_full, groups)):
        t0 = time.time()
        ...
        # -----------------------------------------------------------
        # [vmalYJ STEP A] CONFLICT RESOLUTION on raw targets
        # Using round(3) coarser matching key (catches more synthetic duplicates)
        # -----------------------------------------------------------
        feature_cols_for_conflict = [
            c for c in conflict_key_cols
            if c not in CAT_FEATURES
            and c not in ['lat_decimal_len', 'lon_decimal_len']
        ]
        cat_cols_for_conflict = [c for c in conflict_key_cols if c in CAT_FEATURES]

        # Build string key: round(3) for numerics, raw string for categoricals
        num_part = tr_rows[feature_cols_for_conflict].round(3).astype(str).apply('|'.join, axis=1)
        cat_part = tr_rows[cat_cols_for_conflict].astype(str).fillna('missing').apply('|'.join, axis=1)
        conflict_key = num_part + "||" + cat_part

        group_counts  = tr_rows.groupby(conflict_key)[TARGET_RAW].transform('count')
        group_medians = tr_rows.groupby(conflict_key)[TARGET_RAW].transform('median')
        # Replace targets for conflicting groups with their median
        tr_rows[TARGET_RAW] = np.where(group_counts > 1, group_medians, tr_rows[TARGET_RAW])

        n_conflicts = int((group_counts > 1).sum())
        print(f"      [CONFLICT] Fold {fold+1}: {n_conflicts} rows resolved via coarse round(3) key")

        # -----------------------------------------------------------
        # [vmalYJ STEP B] FIT YEO-JOHNSON ON THIS FOLD's REAL TARGETS
        # Fitted only on real (non-pseudo) tr_rows targets to prevent leakage
        # -----------------------------------------------------------
        real_tr_mask = tr_rows['is_pseudo'] == 0
        pt = PowerTransformer(method='yeo-johnson')
        pt.fit(tr_rows.loc[real_tr_mask, TARGET_RAW].values.reshape(-1, 1))

        # Apply transform to ALL tr_rows (real + pseudo) for model training
        tr_rows[TARGET_RAW] = pt.transform(tr_rows[[TARGET_RAW]]).flatten()

        # Keep va_rows targets in ORIGINAL space for OOF evaluation
        # (inverse-transform predictions to match)

        print(f"      [YJ]       Fold {fold+1}: lambda={pt.lambdas_[0]:.4f}, "
              f"tr_target_range=[{tr_rows[TARGET_RAW].min():.3f}, {tr_rows[TARGET_RAW].max():.3f}]")

        # -----------------------------------------------------------
        # TARGET ENCODINGS (computed on REAL tr_rows, using transformed targets)
        # Stats are in YJ-transformed space — this is intentional so that
        # the target encodings reflect the transformed distribution the models see
        # -----------------------------------------------------------
        real_tr_rows = tr_rows[tr_rows['is_pseudo'] == 0]
        # Global stats in transformed space for this fold
        fold_global_mean   = float(real_tr_rows[TARGET_RAW].mean())
        fold_global_std    = float(real_tr_rows[TARGET_RAW].std())
        fold_global_median = float(real_tr_rows[TARGET_RAW].median())
        fold_global_q25    = float(real_tr_rows[TARGET_RAW].quantile(0.25))
        fold_global_q75    = float(real_tr_rows[TARGET_RAW].quantile(0.75))

        all_te_cols = TARGET_ENC_COLS + COMPOSITE_ENC_COLS
        for col in all_te_cols:
            group_stats = real_tr_rows.groupby(col)[TARGET_RAW].agg(
            ...
            smoothed_median = (group_stats['count'] * group_stats['median'] + s * fold_global_median) / (group_stats['count'] + s)
            smoothed_mean   = (group_stats['count'] * group_stats['mean']   + s * fold_global_mean)   / (group_stats['count'] + s)
            smoothed_std    = (group_stats['count'] * group_stats['std']    + s * fold_global_std)    / (group_stats['count'] + s)
            smoothed_q25    = (group_stats['count'] * group_stats['q25']    + s * fold_global_q25)    / (group_stats['count'] + s)
            smoothed_q75    = (group_stats['count'] * group_stats['q75']    + s * fold_global_q75)    / (group_stats['count'] + s)
            log_count       = np.log1p(group_stats['count'])

            for tgt_df in [tr_rows, va_rows, test_df]:
                tgt_df[f"{col}_target_enc"] = tgt_df[col].astype(str).map(smoothed_median).fillna(fold_global_median).astype(float)
                if col not in COMPOSITE_ENC_COLS:
                    tgt_df[f"{col}_target_q25"] = tgt_df[col].astype(str).map(smoothed_q25).fillna(fold_global_q25).astype(float)
                    tgt_df[f"{col}_target_q75"] = tgt_df[col].astype(str).map(smoothed_q75).fillna(fold_global_q75).astype(float)
                    tgt_df[f"{col}_target_cnt"] = tgt_df[col].astype(str).map(log_count).fillna(0.0).astype(float)
                if col in STD_ENC_COLS:
                    tgt_df[f"{col}_target_std"] = tgt_df[col].astype(str).map(smoothed_std).fillna(fold_global_std).astype(float)
        ...
        # Use YJ-transformed y_tr for model fitting; original y_va for OOF eval
        y_tr = tr_rows[TARGET_RAW]           # YJ-transformed
        y_va = va_rows[TARGET_RAW].values     # original [0,1]
        ...
        # We need YJ-transformed validation targets for early stopping
        y_va_yj = pt.transform(y_va.reshape(-1, 1)).flatten()
        ...
        xgb_m1.fit(X_tr_xgb, y_tr, eval_set=[(X_va_xgb, y_va_yj)], verbose=False)
        ...
        cat_m1.fit(X_tr_cat, y_tr, cat_features=cat_cols,
                   eval_set=(X_va_cat, y_va_yj), early_stopping_rounds=150, verbose=False)
        ...
        lgb_m1.fit(X_tr, y_tr, eval_set=[(X_va, y_va_yj)],
        ...
        xgb_m2.fit(X_tr_xgb, y_tr, eval_set=[(X_va_xgb, y_va_yj)], verbose=False)

        # -----------------------------------------------------------
        # INVERSE-TRANSFORM predictions back to [0,1] original space
        # -----------------------------------------------------------
        def inv(raw_preds):
            return np.clip(pt.inverse_transform(raw_preds.reshape(-1, 1)).flatten(), 0.0, 1.0)

        oof_preds["XGB-MAE-1 (d7)"][va_idx_clean] = inv(xgb_m1.predict(X_va_xgb))
        oof_preds["CAT-MAE-1 (d5)"][va_idx_clean] = inv(cat_m1.predict(X_va_cat))
        oof_preds["CAT-MAE-2 (d5)"][va_idx_clean] = inv(cat_m2.predict(X_va_cat))
        oof_preds["CAT-RMSE (d5)"][va_idx_clean]  = inv(cat_rmse.predict(X_va_cat))
        oof_preds["LGB-MAE (d5)"][va_idx_clean]   = inv(lgb_m1.predict(X_va))
        oof_preds["XGB-MAE-2 (d5)"][va_idx_clean] = inv(xgb_m2.predict(X_va_xgb))

        tst_preds["XGB-MAE-1 (d7)"] += inv(xgb_m1.predict(X_te_xgb))  / N_FOLDS
        tst_preds["CAT-MAE-1 (d5)"] += inv(cat_m1.predict(X_te_cat))   / N_FOLDS
        tst_preds["CAT-MAE-2 (d5)"] += inv(cat_m2.predict(X_te_cat))   / N_FOLDS
        tst_preds["CAT-RMSE (d5)"]  += inv(cat_rmse.predict(X_te_cat)) / N_FOLDS
        tst_preds["LGB-MAE (d5)"]   += inv(lgb_m1.predict(X_te))       / N_FOLDS
        tst_preds["XGB-MAE-2 (d5)"] += inv(xgb_m2.predict(X_te_xgb))  / N_FOLDS

        # Fold OOF metrics in original [0,1] space
        oof_avg_fold = np.mean([oof_preds[m][va_idx_clean] for m in MODEL_NAMES], axis=0)
        f_mae  = mean_absolute_error(y_va, oof_avg_fold)
        f_rmse = root_mean_squared_error(y_va, oof_avg_fold)
        f_ev   = explained_variance_score(y_va, oof_avg_fold)
        print(f"      Fold {fold+1}/5 | XGB1={xgb_m1.best_iteration:<4} CAT1={cat_m1.best_iteration_:<4} "
              f"CAT2={cat_m2.best_iteration_:<4} CAT3={cat_rmse.best_iteration_:<4} "
              f"LGB={lgb_m1.best_iteration_:<4} XGB2={xgb_m2.best_iteration:<4} "
              f"| [ENS MAE={f_mae:.4f} EV={f_ev:.4f}] [{time.time()-t0:.0f}s]")

    real_mask_seed = train_df['is_pseudo'] == 0
    oof_meta_seed = np.column_stack([oof_preds[m][real_mask_seed] for m in MODEL_NAMES])
    ...
    for alpha in [0.001, 0.01, 0.1, 1.0, 10.0]:
        ...
        if cv_score < best_score:
            best_score = cv_score; best_alpha = alpha

    print(f"   [L2 SEARCH SEED {seed}] Best alpha: {best_alpha} (Score: {best_score:.5f})")
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (vmalYJ - Clean Imputation + YJ Transform)")
print("=" * 75)
...
fold_report.to_csv("fold_report_vmalYJ.csv", index=False)
fold_report.to_csv("submissions/fold_report_vmalYJ.csv", index=False)
print(f"\n[DONE] Saved fold_report_vmalYJ.csv")

tst_stacked_avg = np.clip(np.mean(all_tst_stacked, axis=0), 0.0, 1.0)
submission = pd.DataFrame({"record_id": test_df[ID_COL], "flood_risk_score": tst_stacked_avg})
submission.to_csv("submission_vmalYJ.csv", index=False)
submission.to_csv("submissions/submission_vmalYJ.csv", index=False)
print(f"[DONE] Saved submission_vmalYJ.csv ({len(submission)} rows)")

np.save("oof_vmalYJ.npy", all_oof_stacked)
np.save("submissions/oof_vmalYJ.npy", all_oof_stacked)
print(f"[DONE] Saved oof_vmalYJ.npy")

# -----------------------------------------------------------------
# 8. POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (vmalYJ)")
print("=" * 75)

def transform_loss(params):
    a, b, c = params
    pred = np.clip(a * np.power(np.clip(all_oof_stacked, 1e-6, None), b) + c, 0.0, 1.0)
...
opt_oof  = np.clip(a_opt * np.power(np.clip(all_oof_stacked, 1e-6, None), b_opt) + c_opt, 0.0, 1.0)
...
np.save("oof_vmalYJ_optimized.npy", opt_oof)
np.save("submissions/oof_vmalYJ_optimized.npy", opt_oof)

opt_test_preds = np.clip(a_opt * np.power(np.clip(tst_stacked_avg, 1e-6, None), b_opt) + c_opt, 0.0, 1.0)
submission_opt = pd.DataFrame({"record_id": test_df[ID_COL], "flood_risk_score": opt_test_preds})
submission_opt.to_csv("submission_vmalYJ_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_vmalYJ_optimized.csv", index=False)
print(f"[DONE] Saved submission_vmalYJ_optimized.csv ({len(submission_opt)} rows)")
