"""
ML Opsidian: Genesis v703_hub_oof_te - v703 with Huber and Leakage-Free OOF Target Encoding
==========================================================================================
Base: v703_hub (CAT-HUBER replacing CAT-RMSE, 3 Seeds)
Adds:
  - Out-of-Fold (OOF) Target Encoding on the training set to prevent self-leakage.
"""
...
print("=" * 85)
print("  ML OPSIDIAN v703_hub_oof_te - v703 + CAT-HUBER + OOF TE (3 SEEDS)")
print("=" * 85)
...
print("\n[DENOISE] Loading v703 optimized OOF predictions for Self-Distillation...")
oof_v703 = np.load("submissions/oof_v703_optimized.npy")
real_mask_full = train_df['is_pseudo'] == 0
denoised_y = (0.3 * original_target[real_mask_full]) + (0.7 * oof_v703)
train_df.loc[real_mask_full, TARGET] = denoised_y
y = train_df[TARGET]
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v703_hub_oof_te PIPELINE")
print("=" * 75)
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v703_hub_oof_te - v703 with Huber & OOF TE)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v703_hub_oof_te.csv", index=False)
fold_report.to_csv("submissions/fold_report_v703_hub_oof_te.csv", index=False)
...
submission.to_csv("submission_v703_hub_oof_te.csv", index=False)
submission.to_csv("submissions/submission_v703_hub_oof_te.csv", index=False)
print(f"[DONE] Saved submission_v703_hub_oof_te.csv ({len(submission)} rows)")

np.save("oof_v703_hub_oof_te.npy", all_oof_stacked)
np.save("submissions/oof_v703_hub_oof_te.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v703_hub_oof_te.npy")

# -----------------------------------------------------------------
# 8. POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v703_hub_oof_te)")
print("=" * 75)
...
np.save("oof_v703_hub_oof_te_optimized.npy", opt_oof)
np.save("submissions/oof_v703_hub_oof_te_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v703_hub_oof_te_optimized.npy")
...
submission_opt.to_csv("submission_v703_hub_oof_te_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v703_hub_oof_te_optimized.csv", index=False)
print(f"[DONE] Saved submission_v703_hub_oof_te_optimized.csv ({len(submission_opt)} rows)")
