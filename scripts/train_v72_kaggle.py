"""
ML Opsidian: Genesis v72 - 3-Seed Upgraded Smoothing & Stacking Pipeline
==================================================================================
Features of v72:
Inherits v70 (3-Seed Stacking Pipeline) with three modifications:
1. Upgraded Pseudo-Label Source: Uses submission_v67_optimized.csv (LB 0.38216) instead of v30.
2. Survey Categorical Conflict Key: Groups targets by the 11 key survey categoricals rather than 72 exact features.
3. Separate Target Smoothing: Performs conflict resolution on real and pseudo-labeled rows independently.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v72 - UPGRADED SMOOTHING & STACKING PIPELINE")
print("=" * 75)
...
if USE_PSEUDO:
    # Upgrade pseudo-label source to the best LB predictions (v67_optimized)
    pseudo_path = "submission_v67_optimized.csv"
    if not os.path.exists(pseudo_path):
        pseudo_path = "submissions/submission_v67_optimized.csv"
    ...
    if os.path.exists(pseudo_path):
        ...
        print("   [WARNING] submission_v67_optimized.csv not found. Skipping pseudo-labeling.")
...
# Smarter conflict resolution columns (only the categorical answers)
conflict_key_cols = [
    "district", "landcover", "soil_type", "water_supply", 
    "electricity", "road_quality", "urban_rural", 
    "water_presence_flag", "flood_occurrence_current_event", 
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v72 PIPELINE")
print("=" * 75)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        if len(real_tr_idx) > 0:
            ...
            temp_tr_real = temp_tr.loc[real_tr_idx]
            group_medians_real = real_tr_rows.groupby([temp_tr_real[c] for c in temp_tr_real.columns])[TARGET].transform('median')
            tr_rows.loc[real_tr_idx, TARGET] = group_medians_real
            
        if len(pseudo_tr_idx) > 0:
            pseudo_tr_rows = tr_rows.loc[pseudo_tr_idx].copy()
            temp_tr_pseudo = temp_tr.loc[pseudo_tr_idx]
            group_medians_pseudo = pseudo_tr_rows.groupby([temp_tr_pseudo[c] for c in temp_tr_pseudo.columns])[TARGET].transform('median')
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v72 - Raw Custom Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v72.csv", index=False)
fold_report.to_csv("submissions/fold_report_v72.csv", index=False)
...
submission.to_csv("submissions/submission_v72.csv", index=False)
print(f"[DONE] Saved submission_v72.csv ({len(submission)} rows)")

np.save("oof_v72.npy", all_oof_stacked)
np.save("submissions/oof_v72.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v72.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v72)")
print("=" * 75)
...
np.save("oof_v72_optimized.npy", opt_oof)
np.save("submissions/oof_v72_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v72_optimized.npy")
...
submission_opt.to_csv("submission_v72_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v72_optimized.csv", index=False)
print(f"[DONE] Saved submission_v72_optimized.csv ({len(submission_opt)} rows)")
