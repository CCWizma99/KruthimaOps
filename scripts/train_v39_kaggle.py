"""
ML Opsidian: Genesis v39 - Quantile Spline & Downstream Signal Calibration Pipeline
==================================================================================
Features of v39:
1. Multi-Seed Loop (4 Seeds): Runs Level-1 training and Level-2 stacked optimization 
   across seeds [42, 123, 456, 789] to damp random split noise.
2. Downstream Signal Amplification (Track B):
   - confirmed_severe_risk: yes flood + no good to live + non-zero inundation
   - no_flood_confirmed: no flood + yes good to live (low risk anchor)
   - inundation_per_capita: normalized exposure
   - downstream_risk_count: count score of risk features (treated as categorical + target enc)
3. Model Ensemble Matrix:
   - XGB-MAE (d7) with max_delta_step=1
   - CAT-MAE-1 (d5)
   - CAT-MAE-2 (d5)
   - CAT-RMSE (d5)
   - LGB-MAE (d5)
   - LGB-DART (d5) [Fixed 1000 estimators for dropout regularization]
4. L2 Regularized Custom Stacker with Inner CV Grid Search.
5. Quantile Spline Calibration (Track A):
   - Fit scipy.interpolate.PchipInterpolator to map OOF to true target CDF.
6. Post-Hoc Power Transformation on top of spline calibration.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v39 - QUANTILE CALIBRATION & DOWNSTREAM ENGINE")
print("=" * 75)
...
def engineer_features(df):
    ...
    # New downstream features (Track B)
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v39 ENGINE")
print("=" * 75)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        # Re-apply categorical casting to prevent slice-induced dtype shifts
        for col in cat_feature_names:
        ...
        # Unweighted baseline training (weight = 1.0)
        ...
        # === 1. XGBoost with MAE Objective (Native Categoricals) & max_delta_step=1 ===
        ...
        xgb_mae.fit(X_tr, y_tr, eval_set=[(X_va, y_va)], verbose=False)
        oof_preds["XGB-MAE (d7)"][va_idx_clean] = xgb_mae.predict(X_va)
        tst_preds["XGB-MAE (d7)"] += xgb_mae.predict(X_te) / N_FOLDS
        ...
        # === 6. LightGBM DART variant (no early stopping) ===
        ...
        oof_preds["LGB-DART (d5)"][va_idx_clean] = lgb_dart.predict(X_va)
        tst_preds["LGB-DART (d5)"] += lgb_dart.predict(X_te) / N_FOLDS
        ...
        print(f"      Fold {fold+1}/{N_FOLDS} | XGB_it={xgb_mae.best_iteration:<4} CAT1_it={cat_mae1.best_iteration_:<4} CAT2_it={cat_mae2.best_iteration_:<4} CAT3_it={cat_rmse.best_iteration_:<4} LGB_it={lgb_mae.best_iteration_:<4} | [ENS MAE={f_mae:.4f}] [{time.time() - t0:.0f}s]")
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v39 - Raw Custom Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v39.csv", index=False)
fold_report.to_csv("submissions/fold_report_v39.csv", index=False)
...
submission.to_csv("submission_v39.csv", index=False)
submission.to_csv("submissions/submission_v39.csv", index=False)
print(f"[DONE] Saved submission_v39.csv ({len(submission)} rows)")

np.save("oof_v39.npy", all_oof_stacked)
np.save("submissions/oof_v39.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v39.npy")

# -----------------------------------------------------------------
# 8. QUANTILE SPLINE CALIBRATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  QUANTILE SPLINE CALIBRATION (v39)")
print("=" * 75)

def fit_quantile_calibration(oof_preds, y_true):
    percentiles = np.linspace(0.0, 100.0, 200)
    oof_pts = np.percentile(oof_preds, percentiles)
    y_pts = np.percentile(y_true, percentiles)
    
    unique_idx = np.unique(oof_pts, return_index=True)[1]
    unique_idx.sort()
    
    x = oof_pts[unique_idx]
    y_ctrl = y_pts[unique_idx]
    
    if x[0] > 0.0:
        x = np.insert(x, 0, 0.0)
        y_ctrl = np.insert(y_ctrl, 0, 0.0)
    if x[-1] < 1.0:
        x = np.append(x, 1.0)
        y_ctrl = np.append(y_ctrl, 1.0)
        
    for i in range(1, len(x)):
        if x[i] <= x[i-1]:
            x[i] = x[i-1] + 1e-10
            
    pchip = PchipInterpolator(x, y_ctrl)
    return pchip

pchip = fit_quantile_calibration(all_oof_stacked, original_y)
cal_oof = np.clip(pchip(all_oof_stacked), 0.0, 1.0)

cal_mae  = mean_absolute_error(original_y, cal_oof)
cal_rmse = root_mean_squared_error(original_y, cal_oof)
cal_ev   = explained_variance_score(original_y, cal_oof)
cal_lb   = (c_mae * cal_mae + c_rmse * cal_rmse) * (1.0 + c_ev * (1.0 - cal_ev))

print(f"Quantile Spline OOF Metrics:")
print(f"  MAE            : {cal_mae:.5f}")
print(f"  RMSE           : {cal_rmse:.5f}")
print(f"  Explained Var. : {cal_ev:.5f}")
print(f"  Est. LB Score  : {cal_lb:.5f}")

# Apply same spline calibration to test
cal_test_preds = np.clip(pchip(tst_stacked_avg), 0.0, 1.0)

# -----------------------------------------------------------------
# 9. INTEGRATED POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION ON SPLINE CALIBRATION")
print("=" * 75)

def transform_loss(params):
    a, b, c = params
    pred = a * np.power(np.clip(cal_oof, 1e-6, None), b) + c
    pred = np.clip(pred, 0.0, 1.0)
...
opt_oof = a_opt * np.power(np.clip(cal_oof, 1e-6, None), b_opt) + c_opt
...
opt_test_preds = a_opt * np.power(np.clip(cal_test_preds, 1e-6, None), b_opt) + c_opt
...
print(f"[DONE] Saved submission_v39_optimized.csv ({len(submission_opt)} rows)")
