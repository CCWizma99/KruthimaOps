"""
ML Opsidian: Genesis v38 - Regularized Custom Metric Ensemble Pipeline
========================================================================
Features of v38:
1. Multi-Seed Loop (4 Seeds): Runs Level-1 training and Level-2 stacked optimization 
   across seeds [42, 123, 456, 789] to match the variance-dampening performance of v33.
...
5. L2 Regularized Custom Stacker: scipy L-BFGS-B optimizer fitting weights and intercept to 
   directly minimize the competition metric + L2 Ridge weight penalty, preventing models 
   from being zeroed out.
6. Optimal Alpha Grid Search: Inner-loop CV grid search to find the mathematically optimal 
   regularization strength per seed.
7. Integrated Post-Hoc Power Transformation: Calibrates the averaged stacked predictions 
   directly on the target distribution to maximize EV and minimize errors.
8. Constraint compliance: Unweighted training (1.0), GroupKFold spatial partitioning 
...
print("=" * 75)
print("  ML OPSIDIAN v38 - REGULARIZED CUSTOM METRIC ENSEMBLE")
print("=" * 75)
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED REGULARIZED CUSTOM METRIC PIPELINE (v38)")
print("=" * 75)
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v38 - Averaged Multi-Seed Regularized Custom Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v38.csv", index=False)
fold_report.to_csv("submissions/fold_report_v38.csv", index=False)
...
submission.to_csv("submission_v38.csv", index=False)
submission.to_csv("submissions/submission_v38.csv", index=False)
print(f"[DONE] Saved submission_v38.csv ({len(submission)} rows)")

# Save OOF (all_oof_stacked has exactly 20881 rows corresponding to train.csv)
np.save("oof_v38.npy", all_oof_stacked)
np.save("submissions/oof_v38.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v38.npy and submissions/oof_v38.npy (for evaluate.py)")

# -----------------------------------------------------------------
# 8. INTEGRATED POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v38)")
print("=" * 75)
...
submission_opt.to_csv("submission_v38_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v38_optimized.csv", index=False)
print(f"[DONE] Saved submission_v38_optimized.csv ({len(submission_opt)} rows)")
