"""
ML Opsidian: Genesis v40 - Ultimate Fingerprint & Downstream Engine Pipeline
===========================================================================
Features of v40:
...
2. Coordinate Precision Fingerprints Restored (Track F):
   - lat_decimal_len and lon_decimal_len are active features in BASE_FEATURES.
   - Differentiates synthetic coordinate grids from real coordinate precision.
3. Downstream Signal Amplification (Track B):
...
6. Post-Hoc Power Transformation: Calibrates the averaged stacked predictions 
...
print("=" * 75)
print("  ML OPSIDIAN v40 - FINGERPRINT & DOWNSTREAM ENGINE")
print("=" * 75)
...
# Track F: Removed lat_decimal_len and lon_decimal_len from IGNORE_COLS to restore them to BASE_FEATURES
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v40 FINGERPRINT ENGINE")
print("=" * 75)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        # Unweighted training (weight = 1.0)
        ...
        # === 1. XGBoost with MAE Objective & max_delta_step=1 ===
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v40 - Raw Custom Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v40.csv", index=False)
fold_report.to_csv("submissions/fold_report_v40.csv", index=False)
...
submission.to_csv("submissions/submission_v40.csv", index=False)
print(f"[DONE] Saved submission_v40.csv ({len(submission)} rows)")

np.save("oof_v40.npy", all_oof_stacked)
np.save("submissions/oof_v40.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v40.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v40)")
print("=" * 75)
...
submission_opt.to_csv("submission_v40_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v40_optimized.csv", index=False)
print(f"[DONE] Saved submission_v40_optimized.csv ({len(submission_opt)} rows)")
