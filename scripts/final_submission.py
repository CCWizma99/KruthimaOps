...
ML Opsidian: Genesis - Final Submission
=======================================
Strategy:
  - Extract the 802 non-synthetic ground-truth rows from train.csv.
  - Drop the 20,084 synthetic noise rows that were destroying EV.
  - Train optimal tree ensemble exclusively on the real data.
  - Predict on the 5300 test rows (Kaggle backend will automatically score only real test rows).
"""
...
# ==========================================
# We isolate the 802 rows of real data.
train_df = raw_train[raw_train["is_synthetic"].isna()].copy().reset_index(drop=True)
test_df  = raw_test.copy()

print(f"[LOAD] Loading data...")
print(f"   Train original : {raw_train.shape}")
print(f"   Train isolated : {train_df.shape}  <-- Only real data")
...
tst_ensemble_cal = cal.predict(tst_ensemble)
...
sub = pd.DataFrame({
    ...
    TARGET: tst_ensemble_cal
})
sub.to_csv("final_submission.csv", index=False)

pd.DataFrame(fold_results).to_csv("fold_report_final.csv", index=False)

print(f"\n[DONE] Submission -> final_submission.csv  ({len(sub)} rows)")
print(f"       Pred range : [{tst_ensemble_cal.min():.4f}, {tst_ensemble_cal.max():.4f}]  mean={tst_ensemble_cal.mean():.4f}")
print(f"       Fold report -> fold_report_final.csv")
