"""
ML Opsidian: Genesis v36 - Cleaned Champion v30 Stacking Pipeline
========================================================================
Features of v36:
1. Revert to XGB-MAE (d7): Replaces XGB-Custom to resolve the early-stopping/gradient-scale issue.
2. Loss Symmetry Model Matrix: Re-introduces CAT-RMSE (d5) to complement XGB-MAE, CAT-MAE-1, CAT-MAE-2, and LGB-MAE.
3. Level-2 OLS Stacking: Uses LinearRegression(positive=True) fit strictly on original rows (matching v30) to prevent Ridge alpha from over-shrinking estimators to 0.0.
4. Clean Rollback: Unweighted training (1.0 weight) to avoid geospatial imputation overfitting.
5. Precision Fingerprints: Kept in IGNORE_COLS to prevent coordinate decimal-length memorization on test.
6. Center Pseudo-Labeling: Restored from v30 with the 0.46-0.49 confidence window using submission_v30.csv.
"""
...
print("=" * 70)
print("  ML OPSIDIAN v36 - CLEANED CHAMPION v30 PIPELINE")
print("=" * 70)
...
print("\n" + "=" * 70)
print(f"  5-FOLD SPATIAL GROUP CV - SINGLE-SEED CLEANED v30 PIPELINE (v36)")
print("=" * 70)
...
for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
    ...
    print(f"      Fold {fold+1}/{N_FOLDS} | XGB_M_it={xgb_mae.best_iteration:<4} CAT_M1_it={cat_mae1.best_iteration_:<4} CAT_M2_it={cat_mae2.best_iteration_:<4} CAT_R_it={cat_rmse.best_iteration_:<4} LGB_M_it={lgb_mae.best_iteration_:<4} | [ENS MAE={f_mae:.4f}] [{time.time() - t0:.0f}s]")

# -----------------------------------------------------------------
# 6. LEVEL-2 STACKING (Leak-Free Nested CV)
# -----------------------------------------------------------------
...
# Leak-Free Level-2 OOF stacking (GroupKFold) using OLS positive linear regression (matching v30)
...
# Fit final stacker on full meta-features to predict test
...
print("\n" + "=" * 70)
print("  GLOBAL OOF RESULTS (v36 - Cleaned Champion v30 Pipeline)")
print("=" * 70)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v36.csv", index=False)
fold_report.to_csv("submissions/fold_report_v36.csv", index=False)
...
submission.to_csv("submission_v36.csv", index=False)
submission.to_csv("submissions/submission_v36.csv", index=False)
...
np.save("oof_v36.npy", all_oof_stacked)
np.save("submissions/oof_v36.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v36.npy and submissions/oof_v36.npy (for evaluate.py)")
