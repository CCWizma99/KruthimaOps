...
print("=" * 75)
print("  ML OPSIDIAN v1100 - PRUNED & IMPUTED ARCHITECTURE")
print("=" * 75)
...
# -----------------------------------------------------------------
# 2. GEOSPATIAL IMPUTATION & MISSING ARTIFACTS
# -----------------------------------------------------------------
print("\n[FEAT] Creating missing value artifacts before imputation...")
for df in [train_df, test_df]:
    for col in ['latitude', 'longitude', 'elevation_m', 'distance_to_river_m']:
        if col in df.columns:
...
TARGET_ENC_COLS = [
    ...
    "landcover", "soil_type", "downstream_risk_count", "grid_id_050", "grid_id_025"
]
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - SINGLE-SEED v1100 PIPELINE")
print("=" * 75)
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v1100 - Pruned & Imputed)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v1100.csv", index=False)
fold_report.to_csv("submissions/fold_report_v1100.csv", index=False)
...
submission.to_csv("submission_v1100.csv", index=False)
submission.to_csv("submissions/submission_v1100.csv", index=False)
print(f"[DONE] Saved submission_v1100.csv ({len(submission)} rows)")

np.save("oof_v1100.npy", all_oof_stacked)
np.save("submissions/oof_v1100.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v1100.npy")

# -----------------------------------------------------------------
# 8. POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v1100)")
print("=" * 75)
...
np.save("oof_v1100_optimized.npy", opt_oof)
np.save("submissions/oof_v1100_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v1100_optimized.npy")
...
submission_opt.to_csv("submission_v1100_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v1100_optimized.csv", index=False)
print(f"[DONE] Saved submission_v1100_optimized.csv ({len(submission_opt)} rows)")
