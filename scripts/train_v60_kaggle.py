"""
ML Opsidian: Genesis v60 - Refined Custom Stacking Pipeline
===========================================================
Refinements over v57:
1. Expanded 7-Model Stack:
   - Adds LGB-RMSE (d5) with leaf-wise RMSE loss.
   - Increases diversity by combining leaf-wise structure with symmetric trees and depth-wise trees.
2. Blended Pseudo-Labels:
   - Uses a soft pseudo-label source by blending the two best Public LB submissions:
     0.7 * submission_v42_optimized.csv + 0.3 * submission_v45_optimized.csv.
   - Reduces target noise on the test coordinates.
3. Interaction Features:
   - historical_flood_rain_interaction: historical_flood_count * rainfall_7d_mm_log1p.
   - ndwi_rain_interaction: ndwi_qmap * rainfall_7d_mm_log1p.
4. Multi-Start Global Calibration:
   - Runs L-BFGS-B from 5 diverse initial states to find the absolute global minimum of the calibration loss curve.
5. EV-Prioritizing Stacker & Calibration:
   - Keeps c_mae = 0.392696, c_rmse = 0.875527, c_ev = 0.406963.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v60 - REFINED CUSTOM STACKING PIPELINE")
print("=" * 75)
...
if USE_PSEUDO:
    # Blend the two best Public LB submissions (v42_opt and v45_opt)
    path_v42 = "submission_v42_optimized.csv"
    if not os.path.exists(path_v42):
        path_v42 = "submissions/submission_v42_optimized.csv"
        
    path_v45 = "submission_v45_optimized.csv"
    if not os.path.exists(path_v45):
        path_v45 = "submissions/submission_v45_optimized.csv"
        
    if os.path.exists(path_v42) and os.path.exists(path_v45):
        sub_v42 = pd.read_csv(path_v42)
        sub_v45 = pd.read_csv(path_v45)
        sub_blend = sub_v42.copy()
        sub_blend["flood_risk_score"] = 0.7 * sub_v42["flood_risk_score"] + 0.3 * sub_v45["flood_risk_score"]
        
        test_pseudo = test_df.merge(sub_blend, on="record_id", how="left")
        pseudo_rows = test_pseudo.copy()
        pseudo_rows['is_pseudo'] = 1
        
        print(f"   Added {len(pseudo_rows)} soft pseudo-labeled rows from blended v42_opt (70%) and v45_opt (30%).")
        train_df = pd.concat([train_df, pseudo_rows], ignore_index=True)
    else:
        # Fallback to single file if one is missing
        fallback_path = path_v42 if os.path.exists(path_v42) else (path_v45 if os.path.exists(path_v45) else None)
        if fallback_path is None:
            fallback_path = "submission_v42.csv"
            if not os.path.exists(fallback_path):
                fallback_path = "submissions/submission_v42.csv"
        if os.path.exists(fallback_path):
            sub_blend = pd.read_csv(fallback_path)
            test_pseudo = test_df.merge(sub_blend, on="record_id", how="left")
            pseudo_rows = test_pseudo.copy()
            pseudo_rows['is_pseudo'] = 1
            print(f"   Added {len(pseudo_rows)} soft pseudo-labeled rows from fallback {fallback_path}.")
            train_df = pd.concat([train_df, pseudo_rows], ignore_index=True)
        else:
...
def engineer_features(df):
    ...
    # [v60 Refined Interaction Features]
    df["historical_flood_rain_interaction"] = df["historical_flood_count"].fillna(0.0) * df["rainfall_7d_mm_log1p"].fillna(0.0)
    df["ndwi_rain_interaction"] = df["ndwi_qmap"].fillna(0.0) * df["rainfall_7d_mm_log1p"].fillna(0.0)
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - SINGLE-SEED v60 PIPELINE")
print("=" * 75)
...
for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
    ...
    lgb_m2 = lgb.LGBMRegressor(
        n_estimators=4000, learning_rate=0.03, num_leaves=31, max_depth=5,
        min_child_samples=40, subsample=0.8, subsample_freq=1, colsample_bytree=0.6,
        reg_alpha=2.0, reg_lambda=5.0, objective="regression",
        random_state=SEED + 4, n_jobs=-1, verbosity=-1
    )
    lgb_m2.fit(X_tr, y_tr, eval_set=[(X_va, y_va)], categorical_feature=cat_cols, callbacks=[lgb.early_stopping(150, verbose=False)])
    ...
    oof_preds["LGB-RMSE (d5)"][va_idx_clean] = lgb_m2.predict(X_va)
    ...
    tst_preds["LGB-RMSE (d5)"]  += lgb_m2.predict(X_te) / N_FOLDS

    print(f"      Fold {fold+1}/5 | XGB1_it={xgb_m1.best_iteration}  CAT1_it={cat_m1.best_iteration_}  CAT2_it={cat_m2.best_iteration_}  LGB1_it={lgb_m1.best_iteration_}  LGB2_it={lgb_m2.best_iteration_}  [{time.time()-t0:.0f}s]")
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v60 - Raw Stacking)")
print("=" * 75)
...
fold_report.to_csv("fold_report_v60.csv", index=False)
fold_report.to_csv("submissions/fold_report_v60.csv", index=False)
...
submission.to_csv("submissions/submission_v60.csv", index=False)
print(f"[DONE] Saved submission_v60.csv ({len(submission)} rows)")

np.save("oof_v60.npy", oof_stacked)
np.save("submissions/oof_v60.npy", oof_stacked)
print(f"[DONE] Saved oof_v60.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED POST-HOC GLOBAL POWER TRANSFORMATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC GLOBAL POWER TRANSFORMATION OPTIMIZATION (v60)")
print("=" * 75)

# Fit global calibration parameters with multi-start
...
for i, guess in enumerate(initial_guesses):
    res_glob = minimize(global_transform_loss, guess, bounds=bounds, method='L-BFGS-B')
    if res_glob.fun < best_loss:
        best_loss = res_glob.fun
        best_params = res_glob.x
    print(f"   Start {i+1} | Guess: {guess} | Loss: {res_glob.fun:.5f} | Params: a={res_glob.x[0]:.4f}, b={res_glob.x[1]:.4f}, c={res_glob.x[2]:.4f}")

a_glob, b_glob, c_glob = best_params
print(f"Optimal Global Calibration Parameters: a={a_glob:.5f}, b={b_glob:.5f}, c={c_glob:.5f} (Loss: {best_loss:.5f})")
...
np.save("oof_v60_optimized.npy", final_oof)
np.save("submissions/oof_v60_optimized.npy", final_oof)
print(f"[DONE] Saved oof_v60_optimized.npy")
...
submission_opt.to_csv("submission_v60_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v60_optimized.csv", index=False)
print(f"[DONE] Saved submission_v60_optimized.csv ({len(submission_opt)} rows)")
