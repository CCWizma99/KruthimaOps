"""
ML Opsidian: Genesis v33 - Multi-Seed Stacking & MAE-Only Champion Ensemble
========================================================================
Features of v33:
1. MAE-Only Ensemble: Dropped all RMSE models (XGB-RMSE, CAT-RMSE, LGB-RMSE) based on negative Ridge coefficients.
2. Target Encodings: Restored grid_id back to TARGET_ENC_COLS (as in the champion v30).
3. Pseudo-Labeling: Restored from v30 with the 0.46-0.49 confidence window.
4. Level-2 Stacking: Implemented per-seed Ridge stacking (fit stacker per seed, then average OOF and test predictions)
   to preserve the seed-level error structures, with positive=True constraint restored to prevent noise sign-flips.
5. Alpha Search: Grid search for Ridge alpha executed per seed.
6. Baseline: Unweighted training (1.0 weight) to establish the clean baseline before trying weighting on this architecture.
7. Output Paths: Saves results to submissions/ folder for evaluate.py.
"""
...
print("=" * 70)
print("  ML OPSIDIAN v33 - MULTI-SEED MAE-ONLY ENSEMBLE PIPELINE")
print("=" * 70)
...
# Restore grid_id back to TARGET_ENC_COLS (as in the champion v30)
TARGET_ENC_COLS = [
...
# -----------------------------------------------------------------
# Dropped all RMSE models — Ridge confirmed they are net negative
MODEL_NAMES = [
    "XGB-MAE (d7)",
    "CAT-MAE-1 (d5)",
    "CAT-MAE-2 (d5)",
...
print("\n" + "=" * 70)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED MAE-ONLY ENSEMBLE (v33)")
print("=" * 70)
...
print("\n" + "=" * 70)
print("  GLOBAL OOF RESULTS (v33 - MAE-Only Multi-Seed Ensemble)")
print("=" * 70)
print(f"    [ALL ROWS]")
...
# We can create a report based on the average OOF stacked predictions
fold_results_all = []
for fold, (tr_idx, va_idx) in enumerate(gkf_l2.split(original_df, original_y, original_groups)):
...
fold_report.to_csv("fold_report_v33.csv", index=False)
fold_report.to_csv("submissions/fold_report_v33.csv", index=False)
...
submission.to_csv("submissions/submission_v33.csv", index=False)
...
np.save("oof_v33.npy", all_oof_stacked)
np.save("submissions/oof_v33.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v33.npy and submissions/oof_v33.npy (for evaluate.py)")
