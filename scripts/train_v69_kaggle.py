"""
ML Opsidian: Genesis v69 - Multi-Seed Target Power Transformation Pipeline
==========================================================================
Features of v69:
Inherits v64 (Target Power Transformation Stacking Pipeline with submission_v30.csv pseudo-labels) but:
- Introduces multi-seed ensembling (SEEDS = [42, 100]) to average out variance and improve OOF/test generalization.
- Overwrites the previous v69 attempt which added unbalanced RMSE estimators.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v69 - MULTI-SEED TARGET POWER TRANSFORM PIPELINE")
print("=" * 75)
...
# -----------------------------------------------------------------
# 5. TARGET POWER TRANSFORMATION & CROSS-VALIDATION SETUP
# -----------------------------------------------------------------
# [v69] Keep a copy of original target values for validation evaluation
train_df['original_flood_risk_score'] = train_df[TARGET].copy()

# [v69] Fit and apply Yeo-Johnson PowerTransformer globally
pt = PowerTransformer(method='yeo-johnson')
real_mask = train_df['is_pseudo'] == 0
real_targets = train_df.loc[real_mask, [TARGET]]
pt.fit(real_targets)
train_df[TARGET] = pt.transform(train_df[[TARGET]]).flatten()
...
# [v69] original_y and original_df targets remain untransformed/raw
y_arr = train_df['original_flood_risk_score'].values
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v69 PIPELINE")
print("=" * 75)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        # [v69] Predictions - generate in transformed space, and inverse-transform back to original space
        preds_va_xgb1 = xgb_m1.predict(X_va_xgb)
        preds_va_cat1 = cat_m1.predict(X_va_cat)
        preds_va_cat2 = cat_m2.predict(X_va_cat)
        preds_va_rmse = cat_rmse.predict(X_va_cat)
        preds_va_lgb  = lgb_m1.predict(X_va)
        preds_va_xgb2 = xgb_m2.predict(X_va_xgb)

        oof_preds["XGB-MAE-1 (d7)"][va_idx_clean] = pt.inverse_transform(preds_va_xgb1.reshape(-1, 1)).flatten()
        oof_preds["CAT-MAE-1 (d5)"][va_idx_clean] = pt.inverse_transform(preds_va_cat1.reshape(-1, 1)).flatten()
        oof_preds["CAT-MAE-2 (d5)"][va_idx_clean] = pt.inverse_transform(preds_va_cat2.reshape(-1, 1)).flatten()
        oof_preds["CAT-RMSE (d5)"][va_idx_clean] = pt.inverse_transform(preds_va_rmse.reshape(-1, 1)).flatten()
        oof_preds["LGB-MAE (d5)"][va_idx_clean] = pt.inverse_transform(preds_va_lgb.reshape(-1, 1)).flatten()
        oof_preds["XGB-MAE-2 (d5)"][va_idx_clean] = pt.inverse_transform(preds_va_xgb2.reshape(-1, 1)).flatten()

        tst_preds["XGB-MAE-1 (d7)"] += pt.inverse_transform(xgb_m1.predict(X_te_xgb).reshape(-1, 1)).flatten() / N_FOLDS
        tst_preds["CAT-MAE-1 (d5)"] += pt.inverse_transform(cat_m1.predict(X_te_cat).reshape(-1, 1)).flatten() / N_FOLDS
        tst_preds["CAT-MAE-2 (d5)"] += pt.inverse_transform(cat_m2.predict(X_te_cat).reshape(-1, 1)).flatten() / N_FOLDS
        tst_preds["CAT-RMSE (d5)"] += pt.inverse_transform(cat_rmse.predict(X_te_cat).reshape(-1, 1)).flatten() / N_FOLDS
        tst_preds["LGB-MAE (d5)"]  += pt.inverse_transform(lgb_m1.predict(X_te).reshape(-1, 1)).flatten() / N_FOLDS
        tst_preds["XGB-MAE-2 (d5)"] += pt.inverse_transform(xgb_m2.predict(X_te_xgb).reshape(-1, 1)).flatten() / N_FOLDS

        # Evaluate average fold OOF
        oof_avg_fold = np.mean([oof_preds[m][va_idx_clean] for m in MODEL_NAMES], axis=0)
        y_va_arr = va_rows['original_flood_risk_score'].values
        ...
        print(f"      Fold {fold+1}/5 | XGB1_it={xgb_m1.best_iteration:<4} CAT1_it={cat_m1.best_iteration_:<4} CAT2_it={cat_m2.best_iteration_:<4} CAT3_it={cat_rmse.best_iteration_:<4} LGB_it={lgb_m1.best_iteration_:<4} XGB2_it={xgb_m2.best_iteration:<4} | [ENS MAE={f_mae:.4f}] [{time.time() - t0:.0f}s]")

    # Meta features for seed
    ...
    oof_meta_seeds.append(oof_meta_seed)
    tst_meta_seeds.append(tst_meta_seed)

# Average meta features across seeds
oof_meta = np.mean(oof_meta_seeds, axis=0)
tst_meta = np.mean(tst_meta_seeds, axis=0)

# -----------------------------------------------------------------
# Stacking Setup
# -----------------------------------------------------------------
# Inner-Loop CV Grid Search for best custom L2 alpha
print("\n   [STACK] Running nested CV grid search for L2 alpha...")
...
print(f"   [L2 SEARCH] Best alpha: {best_alpha} (Nested CV Metric Score: {best_score:.5f})")
...
for fold, (tr_idx, va_idx) in enumerate(gkf_l2.split(original_df, original_y, original_groups)):
    ...
    all_oof_stacked[va_idx] = np.clip(np.dot(oof_meta[va_idx], w_fold) + b_fold, 0.0, 1.0)
...
tst_stacked_avg = np.clip(np.dot(tst_meta, w_final) + b_final, 0.0, 1.0)
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v69 - Raw Custom Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v69.csv", index=False)
fold_report.to_csv("submissions/fold_report_v69.csv", index=False)
...
submission.to_csv("submissions/submission_v69.csv", index=False)
print(f"[DONE] Saved submission_v69.csv ({len(submission)} rows)")

np.save("oof_v69.npy", all_oof_stacked)
np.save("submissions/oof_v69.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v69.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v69)")
print("=" * 75)
...
np.save("oof_v69_optimized.npy", opt_oof)
np.save("submissions/oof_v69_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v69_optimized.npy")
...
submission_opt.to_csv("submission_v69_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v69_optimized.csv", index=False)
print(f"[DONE] Saved submission_v69_optimized.csv ({len(submission_opt)} rows)")
