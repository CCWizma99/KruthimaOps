"""
ML Opsidian: Genesis v80 - Clean Target Conflict Resolution & Robust 6-Model Stack
==================================================================================
Features of v80:
1. 3-Seed Ensembling: SEEDS = [42, 2024, 7]
2. Clean Target Conflict Resolution (Geospatial Regularization) on real training rows only.
...
5. Revert to robust 6-Model Stack:
   - XGB-MAE-1 (d7)
   - CAT-MAE-1 (d5)
   - CAT-MAE-2 (d5)
   - CAT-RMSE (d5)
...
print("=" * 75)
print("  ML OPSIDIAN v80 - CLEAN PIPELINE WITH 6-MODEL STACK")
print("=" * 75)
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v80 PIPELINE")
print("=" * 75)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        # Clean Target Conflict Resolution: Apply ONLY to the real training rows in the training fold (tr_rows)
        real_tr_mask = tr_rows['is_pseudo'] == 0
        real_tr_rows = tr_rows[real_tr_mask].copy()

        temp_real_tr = real_tr_rows[conflict_key_cols].copy()
        for col in temp_real_tr.columns:
            if temp_real_tr[col].dtype in ['object', 'category']:
                temp_real_tr[col] = temp_real_tr[col].astype(str).fillna('missing')
            else:
                temp_real_tr[col] = temp_real_tr[col].fillna(-999)
                
        group_medians_real = real_tr_rows.groupby([temp_real_tr[c] for c in temp_real_tr.columns])[TARGET].transform('median')
        ...
        print(f"      Fold {fold+1}/5 | XGB1={xgb_m1.best_iteration:<3} CAT1={cat_m1.best_iteration_:<3} CAT2={cat_m2.best_iteration_:<3} CAT3={cat_rmse.best_iteration_:<3} LGB1={lgb_m1.best_iteration_:<3} XGB2={xgb_m2.best_iteration:<3} | [ENS MAE={f_mae:.4f}] [{time.time() - t0:.0f}s]")
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v80 - Clean Custom Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v80.csv", index=False)
fold_report.to_csv("submissions/fold_report_v80.csv", index=False)
...
submission.to_csv("submission_v80.csv", index=False)
submission.to_csv("submissions/submission_v80.csv", index=False)
print(f"[DONE] Saved submission_v80.csv ({len(submission)} rows)")

np.save("oof_v80.npy", all_oof_stacked)
np.save("submissions/oof_v80.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v80.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v80)")
print("=" * 75)
...
np.save("oof_v80_optimized.npy", opt_oof)
np.save("submissions/oof_v80_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v80_optimized.npy")
...
submission_opt.to_csv("submission_v80_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v80_optimized.csv", index=False)
print(f"[DONE] Saved submission_v80_optimized.csv ({len(submission_opt)} rows)")
