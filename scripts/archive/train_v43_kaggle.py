"""
ML Opsidian: Genesis v43 - Iterative Spatial Stacking & Pseudo-Label Refinement Pipeline
=======================================================================================
Features of v43:
1. Iterative Soft Pseudo-Labeling:
   - Appends all 5,300 test rows with soft labels from submission_v42_optimized.csv (LB 0.38245) to training.
   - Marked as is_pseudo = 1, excluded from target encoding, L2 stacker, and OOF validation.
2. 5-Scale Hierarchical Spatial Target Encoding:
   - Target encodes coordinates at 5 hierarchical scales: 1.0, 0.5, 0.25, 0.125, and 0.0625 degrees.
3. Geographic-Environmental Target Encoding Interaction:
   - district_landcover target encoding.
4. Downstream Quad-State Interaction:
...
5. Model Stack:
   ...
   - LGB-MAE (d5)
   - XGB-MAE-2 (d5)
6. L2 Regularized Custom Stacker with Inner CV Grid Search (3 Seeds).
7. Post-Hoc Power Transformation.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v43 - ITERATIVE GEOSPATIAL & TRANSDUCTIVE PIPELINE")
print("=" * 75)
...
if USE_PSEUDO:
    ...
    if os.path.exists(pseudo_path):
        ...
        # Iterative soft pseudo-labeling of all test rows using v42_optimized
        ...
        print("   [WARNING] submission_v42_optimized.csv not found. Skipping pseudo-labeling.")
...
def engineer_features(df):
    ...
    df["grid_id_006"] = (lat / 0.0625).astype(int).astype(str) + "_" + (lon / 0.0625).astype(int).astype(str)
    
    # Geographic-Environmental Interaction
    df["district_landcover"] = df["district"].astype(str) + "_" + df["landcover"].astype(str)
...
TARGET_ENC_COLS = [
    ...
    "downstream_risk_count", "grid_id_100", "grid_id_050", "grid_id_025", "grid_id_012", "grid_id_006",
    "district_landcover"
]
...
# Ignore raw high-cardinality grid helper IDs and interactions in base features
IGNORE_COLS = DROP_COLS + [
    TARGET, "flood_occurrence_yes", "downstream_sig", "downstream_quad_sig", "infra_deficit_sig", "district_landcover"
]
SPATIAL_HELPERS = ["lat_bin", "lon_bin", "grid_id", "grid_id_100", "grid_id_050", "grid_id_025", "grid_id_012", "grid_id_006"]
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v43 PIPELINE")
print("=" * 75)
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v43 - Raw Custom Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v43.csv", index=False)
fold_report.to_csv("submissions/fold_report_v43.csv", index=False)
...
submission.to_csv("submissions/submission_v43.csv", index=False)
print(f"[DONE] Saved submission_v43.csv ({len(submission)} rows)")

np.save("oof_v43.npy", all_oof_stacked)
np.save("submissions/oof_v43.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v43.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v43)")
print("=" * 75)
...
submission_opt.to_csv("submission_v43_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v43_optimized.csv", index=False)
print(f"[DONE] Saved submission_v43_optimized.csv ({len(submission_opt)} rows)")
