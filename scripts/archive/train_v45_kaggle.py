"""
ML Opsidian: Genesis v45 - Raw Pseudo-Label Stacking Pipeline (Anti-Double-Calibration)
======================================================================================
Upgrades from v44:
1. Reverted Pseudo-Label Scale (Fixing Double-Calibration):
   - Uses `submission_v42.csv` (raw, un-calibrated predictions) as the soft pseudo-label source instead of `submission_v42_optimized.csv`.
   - Prevents the tree models from learning post-hoc stretched target distributions on the test coordinates, eliminating target mismatch and double-calibration.
2. Simulator Weights:
   - Uses the 17-point recalibrated weights:
     c_mae = 0.535196, c_rmse = 1.146326, c_ev = 0.054898.
3. Stable Geospatial Features:
   - Maintains the reverted micro-spatial grids (maximum grid resolution of 0.125 degrees).
4. Config:
   - Single seed `SEEDS = [42]` for fast execution.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v45 - ANTI-DOUBLE-CALIBRATION PIPELINE")
print("=" * 75)
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v45 PIPELINE")
print("=" * 75)
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v45 - Raw Custom Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v45.csv", index=False)
fold_report.to_csv("submissions/fold_report_v45.csv", index=False)
...
submission.to_csv("submission_v45.csv", index=False)
submission.to_csv("submissions/submission_v45.csv", index=False)
print(f"[DONE] Saved submission_v45.csv ({len(submission)} rows)")

np.save("oof_v45.npy", all_oof_stacked)
np.save("submissions/oof_v45.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v45.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v45)")
print("=" * 75)
...
np.save("oof_v45_optimized.npy", opt_oof)
np.save("submissions/oof_v45_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v45_optimized.npy")
...
submission_opt.to_csv("submission_v45_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v45_optimized.csv", index=False)
print(f"[DONE] Saved submission_v45_optimized.csv ({len(submission_opt)} rows)")
