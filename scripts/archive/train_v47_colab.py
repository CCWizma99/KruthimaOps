...
==========================================================================
OPTIMIZED FOR GOOGLE COLAB GPU RUNTIME (T4/V100/A100)
...
3. Colab GPU Acceleration:
   - Configures XGBoost on GPU (device="cuda").
   - Configures CatBoost on GPU (task_type="GPU").
   - Runs the full 5 separate CatBoost quantile base models on GPU in seconds.
   - Auto-resolves file paths for direct Colab uploads (/content/).
"""
...
# -----------------------------------------------------------------
# 0. COLAB PATH RESOLUTION
# -----------------------------------------------------------------
# Auto-resolve where the dataset files reside
...
print("=" * 75)
print("  ML OPSIDIAN v47 - GEOGRAPHY & MONSOON COLAB GPU PIPELINE")
print("=" * 75)
...
if USE_PSEUDO:
    # Try multiple common paths for the pseudo-labels file in Colab/local
    pseudo_paths = [
        "submission_v46.csv",
        "submissions/submission_v46.csv",
        "/content/submission_v46.csv",
        "/content/submission_v45.csv",
        "submission_v45.csv",
        "submissions/submission_v45.csv"
    ]
    ...
    if pseudo_path is not None:
        print(f"   Using soft pseudo-label source: {pseudo_path}")
        ...
        print("   [WARNING] No pseudo-label source found (submission_v46.csv or submission_v45.csv). Skipping pseudo-labeling.")
...
# Note: Nominal features are explicitly cast as categories below in the [PREP] block
# after missing values have been filled, preventing TypeError on setitem.
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        # 7. Quantile Ensemble Base Model (Fully GPU Accelerated on CatBoost in Colab)
        quantile_preds_va = []
        quantile_preds_te = []
        for alpha in QUANTILE_ALPHAS:
            qmodel = cb.CatBoostRegressor(
                ...
                task_type=CB_TASK_TYPE, random_seed=seed, verbose=False
            )
            qmodel.fit(X_tr_cat, y_tr, cat_features=cat_cols, eval_set=(X_va_cat, y_va), early_stopping_rounds=100, verbose=False)
        ...
        oof_preds["CAT-Quantile-Median"][va_idx_clean] = va_quantile_ensemble
        ...
        tst_preds["CAT-Quantile-Median"] += te_quantile_ensemble / N_FOLDS
