"""
ML Opsidian: Genesis v73 - Calibrated Metric, Weighted Pseudo-Labeling & Diverse Stack
==================================================================================
Features of v73:
1. 3-Seed Ensembling: SEEDS = [42, 2024, 7]
2. Leak-Free Target Conflict Resolution inside the fold loop.
3. Correct Competition Metric Coefficients inside L2 Stacker & Post-Hoc:
   - c_mae = 0.535196, c_rmse = 1.146326, c_ev = 0.054898
4. Weighted Pseudo-Labeling:
   - Loads soft pseudo-labels from submission_v70_optimized.csv.
   - Assigns sample_weight = 1.0 to real data, 0.8 to pseudo-labeled test data.
5. Expanded Ensemble Stack:
   - XGB-MAE-1 (d7)
   - CAT-MAE-1 (d5)
   - CAT-MAE-2 (d5)
   - CAT-RMSE (d5)
   ...
   - LGB-RMSE (d5) [NEW]
   - XGB-RMSE (d5) [NEW]
6. Relaxed Post-Hoc Calibration bounds.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v73 - CALIBRATED PIPELINE WITH WEIGHTED PSEUDO-LABELS")
print("=" * 75)
...
if USE_PSEUDO:
    pseudo_path = "submission_v70_optimized.csv"
    if not os.path.exists(pseudo_path):
        pseudo_path = "submissions/submission_v70_optimized.csv"
    ...
    if os.path.exists(pseudo_path):
        ...
        pseudo_rows['sample_weight'] = 0.8  # Pseudo-labels are weighted 0.8
        
        print(f"   Added {len(pseudo_rows)} soft pseudo-labeled rows from test set with weight 0.8.")
        train_df = pd.concat([train_df, pseudo_rows], ignore_index=True)
    else:
        print("   [WARNING] submission_v70_optimized.csv not found. Skipping pseudo-labeling.")
...
IGNORE_COLS = DROP_COLS + [
    TARGET, "flood_occurrence_yes", "downstream_sig", "downstream_quad_sig", "infra_deficit_sig", "sample_weight"
]
...
# Pre-processing Conflict Resolution Column Definitions (No global overwrite of train_df[TARGET])
downstream_cols = [
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v73 PIPELINE")
print("=" * 75)

t_start_global = time.time()

# Competition Metric Coefficients (Calibrated)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        w_tr = tr_rows['sample_weight'].values  # Weighted training
        ...
        lgb_rmse.fit(X_tr, y_tr, sample_weight=w_tr, eval_set=[(X_va, y_va)], callbacks=[lgb.early_stopping(150, verbose=False)])
        ...
        xgb_rmse = xgb.XGBRegressor(
            ...
            enable_categorical=False, early_stopping_rounds=100, random_state=seed + 5, n_jobs=-1
        )
        xgb_rmse.fit(X_tr_xgb, y_tr, sample_weight=w_tr, eval_set=[(X_va_xgb, y_va)], verbose=False)
        ...
        oof_preds["XGB-RMSE (d5)"][va_idx_clean] = xgb_rmse.predict(X_va_xgb)
        ...
        tst_preds["XGB-RMSE (d5)"] += xgb_rmse.predict(X_te_xgb) / N_FOLDS
        ...
        print(f"      Fold {fold+1}/5 | XGB1={xgb_m1.best_iteration:<3} CAT1={cat_m1.best_iteration_:<3} CAT2={cat_m2.best_iteration_:<3} CAT3={cat_rmse.best_iteration_:<3} LGB1={lgb_m1.best_iteration_:<3} XGB2={xgb_m2.best_iteration:<3} LGB2={lgb_rmse.best_iteration_:<3} XGB3={xgb_rmse.best_iteration:<3} | [ENS MAE={f_mae:.4f}] [{time.time() - t0:.0f}s]")
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v73 - Raw Custom Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v73.csv", index=False)
fold_report.to_csv("submissions/fold_report_v73.csv", index=False)
...
submission.to_csv("submissions/submission_v73.csv", index=False)
print(f"[DONE] Saved submission_v73.csv ({len(submission)} rows)")

np.save("oof_v73.npy", all_oof_stacked)
np.save("submissions/oof_v73.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v73.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v73)")
print("=" * 75)
...
bounds = [(0.1, 3.0), (0.1, 5.0), (-0.5, 0.5)]  # Relaxed bounds
...
np.save("oof_v73_optimized.npy", opt_oof)
np.save("submissions/oof_v73_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v73_optimized.npy")
...
submission_opt.to_csv("submission_v73_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v73_optimized.csv", index=False)
print(f"[DONE] Saved submission_v73_optimized.csv ({len(submission_opt)} rows)")
