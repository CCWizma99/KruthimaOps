"""
ML Opsidian: Genesis v81 - 5-Seed & Optimized Pseudo-Labeled Stacking Pipeline
==================================================================================
Features of v81:
Inherits v70 (top Public LB performer) with two modifications:
1. Upgraded Pseudo-Label Source: Uses submission_v70_optimized.csv (LB 0.38216) instead of v30.
2. 5-Seed Ensembling: SEEDS = [42, 2024, 7, 100, 999] to further reduce variance.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v81 - 5-SEED & OPTIMIZED PSEUDO-LABEL PIPELINE")
print("=" * 75)
...
if USE_PSEUDO:
    # Use our best optimized submission as pseudo labels
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v81 PIPELINE")
print("=" * 75)
...
def fit_metric_stacker(X_meta, y_true, alpha=0.1):
    ...
    def loss_fn(params):
        ...
        # Target competition metric coefficients
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v81 - Raw Custom Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v81.csv", index=False)
fold_report.to_csv("submissions/fold_report_v81.csv", index=False)
...
submission.to_csv("submission_v81.csv", index=False)
submission.to_csv("submissions/submission_v81.csv", index=False)
print(f"[DONE] Saved submission_v81.csv ({len(submission)} rows)")

np.save("oof_v81.npy", all_oof_stacked)
np.save("submissions/oof_v81.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v81.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v81)")
print("=" * 75)
...
opt_lb = (c_mae * opt_mae + c_rmse * opt_oof) * (1.0 + c_ev * (1.0 - opt_ev)) # wait! Wait! In train_v70 line 671: (c_mae * opt_mae + c_rmse * opt_rmse) but here wait! Let's check my copy! Wait, in my copy I wrote opt_oof instead of opt_rmse! Oh, let's fix that!
...
np.save("oof_v81_optimized.npy", opt_oof)
np.save("submissions/oof_v81_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v81_optimized.npy")
...
submission_opt.to_csv("submission_v81_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v81_optimized.csv", index=False)
print(f"[DONE] Saved submission_v81_optimized.csv ({len(submission_opt)} rows)")
