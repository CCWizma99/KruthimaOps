"""
ML Opsidian: Genesis vmalYJh_s1 - vmalYJ + CAT-HUBER (SINGLE SEED)
==================================================================
Base: vmalYJ (vmal + Yeo-Johnson target transform + 5 Seeds)
Modified to:
  - Run with SEEDS = [42] (Single seed probe).
  - Replace the CAT-RMSE model with the CAT-HUBER (d6) model (which gets active stacker weight).
"""
...
print("=" * 75)
print("  ML OPSIDIAN vmalYJh_s1 — YEO-JOHNSON + CAT-HUBER (SINGLE SEED)")
print("=" * 75)
...
# -----------------------------------------------------------------
# 5. MODEL DEFINITIONS (CAT-HUB replaces CAT-RMSE)
# -----------------------------------------------------------------
MODEL_NAMES = [
    "XGB-MAE-1 (d7)",
    "CAT-MAE-1 (d5)",
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - SINGLE-SEED vmalYJh_s1 PIPELINE")
print("=" * 75)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y_full, groups)):
        ...
        y_tr = tr_rows[TARGET_RAW]
        y_va = va_rows[TARGET_RAW].values
        ...
        cat_hub.fit(X_tr_cat, y_tr, cat_features=cat_cols,
                    eval_set=(X_va_cat, y_va_yj), early_stopping_rounds=150, verbose=False)
        ...
        oof_preds["CAT-HUB (d6)"][va_idx_clean]   = inv(cat_hub.predict(X_va_cat))
        ...
        tst_preds["CAT-HUB (d6)"]   += inv(cat_hub.predict(X_te_cat))  / N_FOLDS
        ...
        print(f"      Fold {fold+1}/5 | XGB1={xgb_m1.best_iteration:<4} CAT1={cat_m1.best_iteration_:<4} "
              f"CAT2={cat_m2.best_iteration_:<4} HUB={cat_hub.best_iteration_:<4} "
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (vmalYJh_s1 - Clean Imputation + YJ Transform + CAT-HUBER)")
print("=" * 75)
...
fold_report.to_csv("fold_report_vmalYJh_s1.csv", index=False)
fold_report.to_csv("submissions/fold_report_vmalYJh_s1.csv", index=False)
print(f"\n[DONE] Saved fold_report_vmalYJh_s1.csv")
...
submission.to_csv("submission_vmalYJh_s1.csv", index=False)
submission.to_csv("submissions/submission_vmalYJh_s1.csv", index=False)
print(f"[DONE] Saved submission_vmalYJh_s1.csv ({len(submission)} rows)")

np.save("oof_vmalYJh_s1.npy", all_oof_stacked)
np.save("submissions/oof_vmalYJh_s1.npy", all_oof_stacked)
print(f"[DONE] Saved oof_vmalYJh_s1.npy")

# -----------------------------------------------------------------
# 8. POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (vmalYJh_s1)")
print("=" * 75)
...
np.save("oof_vmalYJh_s1_optimized.npy", opt_oof)
np.save("submissions/oof_vmalYJh_s1_optimized.npy", opt_oof)
...
submission_opt.to_csv("submission_vmalYJh_s1_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_vmalYJh_s1_optimized.csv", index=False)
print(f"[DONE] Saved submission_vmalYJh_s1_optimized.csv ({len(submission_opt)} rows)")
