"""
ML Opsidian: Genesis v702 - Clean v70 Evolution
==================================================================================
Base: v70 (6-Model, 3-Seed, Broad Conflict-Key Stacking Pipeline)
Exactly 3 changes vs v70 — nothing else touched:
1. Pseudo-Labels: submission_v70.csv (best quality) instead of submission_v30.csv.
2. Clean Conflict Resolution: Group medians computed on real rows (is_pseudo=0) only,
   applied back to real rows only. Pseudo rows retain their original v70 predictions.
3. Spatial Gradient Features: Local-vs-global risk divergence from vc2:
     spatial_grad_012_100 = grid_id_012_target_enc - grid_id_100_target_enc
     spatial_grad_025_050 = grid_id_025_target_enc - grid_id_050_target_enc
"""
...
print("=" * 75)
print("  ML OPSIDIAN v702 - CLEAN v70 EVOLUTION")
print("=" * 75)
...
if USE_PSEUDO:
    # [CHANGE 1] Use best-quality pseudo-labels from v70 (not v30)
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v702 PIPELINE")
print("=" * 75)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        # [CHANGE 2] Clean Conflict Resolution: real rows only, no pseudo contamination
        ...
        group_medians_real = real_tr_rows_cr.groupby(
            [temp_real[c] for c in temp_real.columns]
        )[TARGET].transform('median')
        tr_rows.loc[real_tr_mask, TARGET] = group_medians_real
        # Pseudo rows retain their original v70 predictions untouched
        ...
        # [CHANGE 3] Spatial gradient features: local vs global risk divergence
        for tgt_df in [tr_rows, va_rows, test_df]:
        ...
        # 1. XGB-MAE-1 (d7) — identical to v70
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v702 - Clean v70 Evolution)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v702.csv", index=False)
fold_report.to_csv("submissions/fold_report_v702.csv", index=False)
...
submission.to_csv("submission_v702.csv", index=False)
submission.to_csv("submissions/submission_v702.csv", index=False)
print(f"[DONE] Saved submission_v702.csv ({len(submission)} rows)")

np.save("oof_v702.npy", all_oof_stacked)
np.save("submissions/oof_v702.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v702.npy")

# -----------------------------------------------------------------
# 8. POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v702)")
print("=" * 75)
...
np.save("oof_v702_optimized.npy", opt_oof)
np.save("submissions/oof_v702_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v702_optimized.npy")
...
submission_opt.to_csv("submission_v702_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v702_optimized.csv", index=False)
print(f"[DONE] Saved submission_v702_optimized.csv ({len(submission_opt)} rows)")
