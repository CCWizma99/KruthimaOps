"""
ML Opsidian: Genesis v63 - Upgraded Stacking & Pseudo-Labeling Pipeline
========================================================================
Refinements over v42:
1. Upgraded Pseudo-Label Source:
   - Upgrades soft pseudo-labels source from submission_v30.csv (0.38293)
     to submission_v42_optimized.csv (our best Public LB score at 0.38245).
2. Codebase Integrity:
   - Keeps the exact modeling, feature engineering, custom L2 stacker weights,
     and global power transformation calibration from v42 to preserve the best baseline.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v63 - UPGRADED GEOSPATIAL & TRANSDUCTIVE PIPELINE")
print("=" * 75)
...
if USE_PSEUDO:
    # Upgrade pseudo label source to submission_v42_optimized.csv (our best Public LB score at 0.38245)
    pseudo_path = "submission_v42_optimized.csv"
    if not os.path.exists(pseudo_path):
        pseudo_path = "submissions/submission_v42_optimized.csv"
        
    # Fallback to v30 if missing
    if not os.path.exists(pseudo_path):
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - SINGLE-SEED v63 PIPELINE")
print("=" * 75)
...
# -----------------------------------------------------------------
# Custom stacker metric loss (v42 target weights):
...
for seed in SEEDS:
    ...
    # Inner-Loop CV Grid Search for L2 alpha
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v63 - Raw Custom Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v63.csv", index=False)
fold_report.to_csv("submissions/fold_report_v63.csv", index=False)
...
submission.to_csv("submission_v63.csv", index=False)
submission.to_csv("submissions/submission_v63.csv", index=False)
print(f"[DONE] Saved submission_v63.csv ({len(submission)} rows)")

np.save("oof_v63.npy", all_oof_stacked)
np.save("submissions/oof_v63.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v63.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v63)")
print("=" * 75)
...
np.save("oof_v63_optimized.npy", opt_oof)
np.save("submissions/oof_v63_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v63_optimized.npy")
...
submission_opt.to_csv("submission_v63_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v63_optimized.csv", index=False)
print(f"[DONE] Saved submission_v63_optimized.csv ({len(submission_opt)} rows)")
