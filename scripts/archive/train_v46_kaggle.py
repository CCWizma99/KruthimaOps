"""
ML Opsidian: Genesis v46 - Advanced Refinement Pipeline
======================================================
Upgrades from v45:
1. Leave-One-Out (LOO) Target Encoding:
   - Built inside the CV loop for 'downstream_sig' and 'district'.
   - Real training rows exclude self from stats calculation.
   - Pseudo-labeled test rows are excluded from stats to prevent leakage.
2. Quantile Ensemble Base Model:
   - Trains 5 CatBoost base models at quantiles [0.40, 0.45, 0.50, 0.55, 0.60].
   - Feeds their median prediction as a 7th base model into the Ridge stacker.
3. Fallback-Enabled Per-Group Calibration:
   - Fits separate power transformations (a * pred^b + c) for downstream_sig risk groups.
   - Groups with < 10 samples default to a global power calibration fallback.
4. Analytical Group Median Blend:
   - Computes Bayesian-smoothed group medians out-of-fold using train + pseudo-labeled test rows.
   - Optimizes blend weight BLEND_ALPHA on OOF cross-validation to combine stacker and analytical.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v46 - ADVANCED REFINEMENT PIPELINE")
print("=" * 75)
...
if USE_PSEUDO:
    # Use RAW un-calibrated v45 predictions to prevent double-calibration of test coordinates
    ...
    if os.path.exists(pseudo_path):
        ...
        print("   [WARNING] submission_v42.csv not found. Skipping pseudo-labeling.")
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v46 PIPELINE")
print("=" * 75)
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v46 - Raw Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v46.csv", index=False)
fold_report.to_csv("submissions/fold_report_v46.csv", index=False)
...
submission.to_csv("submission_v46.csv", index=False)
submission.to_csv("submissions/submission_v46.csv", index=False)
print(f"[DONE] Saved submission_v46.csv ({len(submission)} rows)")

np.save("oof_v46.npy", all_oof_stacked)
np.save("submissions/oof_v46.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v46.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED FALLBACK-ENABLED PER-GROUP POWER TRANSFORMATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v46)")
print("=" * 75)
...
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  DYNAMIC ANALYTICAL GROUP MEDIAN BLEND (v46)")
print("=" * 75)
...
np.save("oof_v46_optimized.npy", final_oof)
np.save("submissions/oof_v46_optimized.npy", final_oof)
print(f"[DONE] Saved oof_v46_optimized.npy")
...
submission_opt.to_csv("submission_v46_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v46_optimized.csv", index=False)
print(f"[DONE] Saved submission_v46_optimized.csv ({len(submission_opt)} rows)")
