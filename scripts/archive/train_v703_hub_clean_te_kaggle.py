"""
ML Opsidian: Genesis v703_hub_clean_te - v703 with Huber and Leakage-Free OOF Target Encoding (No Target Denoising)
==================================================================================================================
Base: v703_hub_oof_te (CAT-HUBER, OOF TE, 3 Seeds)
Changes:
  - Disables target denoising (self-distillation) to train on raw y.
"""
...
print("=" * 85)
print("  ML OPSIDIAN v703_hub_clean_te - v703 + CAT-HUBER + OOF TE + Raw Target (3 SEEDS)")
print("=" * 85)
...
def engineer_features(df):
    ...
    df["inundation_per_capita"] = df["inundation_area_sqm"] / (np.expm1(df["population_density_per_capita_log1p"] if "population_density_per_capita_log1p" in df.columns else df["population_density_per_km2_log1p"]) + 1.0)
    ...
    df["infra_resilience"]        = df["infrastructure_score"] / ((df["population_density_per_capita_log1p"] if "population_density_per_capita_log1p" in df.columns else df["population_density_per_km2_log1p"]) + 1e-6)
    df["evacuation_difficulty"]   = df["nearest_hospital_km_log1p"] + df["nearest_evac_km_log1p"]
    df["inundation_density_risk"] = df["inundation_area_log"] / ((df["population_density_per_capita_log1p"] if "population_density_per_capita_log1p" in df.columns else df["population_density_per_km2_log1p"]) + 1e-6)
...
# -----------------------------------------------------------------
# 5. MODEL DEFINITIONS (Huber replaces RMSE)
# -----------------------------------------------------------------
MODEL_NAMES = [
    "XGB-MAE-1 (d7)",
    "CAT-MAE-1 (d5)",
...
# NO TARGET DENOISING / SELF-DISTILLATION
original_target = train_df[TARGET].copy()
...
original_y = original_target[real_mask].values
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v703_hub_clean_te PIPELINE")
print("=" * 75)

t_start_global = time.time()

c_mae, c_rmse, c_ev, c_int = 0.543990, 1.148842, 0.049105, -0.000309
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        # -----------------------------------------------------------------
        # Initialize target encoding columns in tr_rows
        all_te_cols = TARGET_ENC_COLS + COMPOSITE_ENC_COLS
        for col in all_te_cols:
            tr_rows[f"{col}_target_enc"] = np.nan
            if col not in COMPOSITE_ENC_COLS:
                tr_rows[f"{col}_target_q25"] = np.nan
                tr_rows[f"{col}_target_q75"] = np.nan
                tr_rows[f"{col}_target_cnt"] = np.nan
            if col in STD_ENC_COLS:
                tr_rows[f"{col}_target_std"] = np.nan

        real_tr_rows = tr_rows[tr_rows['is_pseudo'] == 0]
        fold_global_mean   = float(real_tr_rows[TARGET].mean())
        fold_global_std    = float(real_tr_rows[TARGET].std())
        fold_global_median = float(real_tr_rows[TARGET].median())
        fold_global_q25    = float(real_tr_rows[TARGET].quantile(0.25))
        fold_global_q75    = float(real_tr_rows[TARGET].quantile(0.75))

        # 1. Map to va_rows, test_df, and pseudo rows (which are leakage-free) using full real_tr_rows stats
        for col in all_te_cols:
            ...
            # Map to va_rows & test_df
            for tgt_df in [va_rows, test_df]:
            ...
            # Map to pseudo rows inside tr_rows
            pseudo_mask = tr_rows['is_pseudo'] == 1
            if pseudo_mask.any():
                tr_rows.loc[pseudo_mask, f"{col}_target_enc"] = tr_rows.loc[pseudo_mask, col].astype(str).map(smoothed_median).fillna(fold_global_median).astype(float)
                if col not in COMPOSITE_ENC_COLS:
                    tr_rows.loc[pseudo_mask, f"{col}_target_q25"] = tr_rows.loc[pseudo_mask, col].astype(str).map(smoothed_q25).fillna(fold_global_q25).astype(float)
                    tr_rows.loc[pseudo_mask, f"{col}_target_q75"] = tr_rows.loc[pseudo_mask, col].astype(str).map(smoothed_q75).fillna(fold_global_q75).astype(float)
                    tr_rows.loc[pseudo_mask, f"{col}_target_cnt"] = tr_rows.loc[pseudo_mask, col].astype(str).map(log_count).fillna(0.0).astype(float)
                if col in STD_ENC_COLS:
                    tr_rows.loc[pseudo_mask, f"{col}_target_std"] = tr_rows.loc[pseudo_mask, col].astype(str).map(smoothed_std).fillna(fold_global_std).astype(float)

        # 2. Map OOF to the real rows in tr_rows
        # 5-fold KFold split on real training rows
        kf_te = KFold(n_splits=5, shuffle=True, random_state=seed)
        real_tr_df = real_tr_rows.copy().reset_index(drop=True)
        
        te_arrays = {f"{col}_target_enc": np.zeros(len(real_tr_df)) for col in all_te_cols}
        te_arrays_q25 = {f"{col}_target_q25": np.zeros(len(real_tr_df)) for col in all_te_cols if col not in COMPOSITE_ENC_COLS}
        te_arrays_q75 = {f"{col}_target_q75": np.zeros(len(real_tr_df)) for col in all_te_cols if col not in COMPOSITE_ENC_COLS}
        te_arrays_cnt = {f"{col}_target_cnt": np.zeros(len(real_tr_df)) for col in all_te_cols if col not in COMPOSITE_ENC_COLS}
        te_arrays_std = {f"{col}_target_std": np.zeros(len(real_tr_df)) for col in all_te_cols if col in STD_ENC_COLS}

        for te_tr_idx, te_va_idx in kf_te.split(real_tr_df):
            sub_tr = real_tr_df.iloc[te_tr_idx]
            sub_va = real_tr_df.iloc[te_va_idx]
            
            sub_median = float(sub_tr[TARGET].median())
            sub_mean   = float(sub_tr[TARGET].mean())
            sub_std    = float(sub_tr[TARGET].std())
            sub_q25    = float(sub_tr[TARGET].quantile(0.25))
            sub_q75    = float(sub_tr[TARGET].quantile(0.75))

            for col in all_te_cols:
                sub_group = sub_tr.groupby(col)[TARGET].agg(
                    median='median', count='count', mean='mean', std='std',
                    q25=lambda x: x.quantile(0.25),
                    q75=lambda x: x.quantile(0.75)
                )
                sub_group['std'] = sub_group['std'].fillna(0.0)
                s = SMOOTHING_COMPOSITE if col in COMPOSITE_ENC_COLS else SMOOTHING

                sub_smoothed_median = (sub_group['count'] * sub_group['median'] + s * sub_median) / (sub_group['count'] + s)
                sub_smoothed_mean   = (sub_group['count'] * sub_group['mean']   + s * sub_mean)   / (sub_group['count'] + s)
                sub_smoothed_std    = (sub_group['count'] * sub_group['std']    + s * sub_std)    / (sub_group['count'] + s)
                sub_smoothed_q25    = (sub_group['count'] * sub_group['q25']    + s * sub_q25)    / (sub_group['count'] + s)
                sub_smoothed_q75    = (sub_group['count'] * sub_group['q75']    + s * sub_q75)    / (sub_group['count'] + s)
                sub_log_count       = np.log1p(sub_group['count'])

                te_arrays[f"{col}_target_enc"][te_va_idx] = sub_va[col].astype(str).map(sub_smoothed_median).fillna(sub_median).values
                if col not in COMPOSITE_ENC_COLS:
                    te_arrays_q25[f"{col}_target_q25"][te_va_idx] = sub_va[col].astype(str).map(sub_smoothed_q25).fillna(sub_q25).values
                    te_arrays_q75[f"{col}_target_q75"][te_va_idx] = sub_va[col].astype(str).map(sub_smoothed_q75).fillna(sub_q75).values
                    te_arrays_cnt[f"{col}_target_cnt"][te_va_idx] = sub_va[col].astype(str).map(sub_log_count).fillna(0.0).values
                if col in STD_ENC_COLS:
                    te_arrays_std[f"{col}_target_std"][te_va_idx] = sub_va[col].astype(str).map(sub_smoothed_std).fillna(sub_std).values

        real_mask_idx = tr_rows['is_pseudo'] == 0
        for col in all_te_cols:
            tr_rows.loc[real_mask_idx, f"{col}_target_enc"] = te_arrays[f"{col}_target_enc"]
            if col not in COMPOSITE_ENC_COLS:
                tr_rows.loc[real_mask_idx, f"{col}_target_q25"] = te_arrays_q25[f"{col}_target_q25"]
                tr_rows.loc[real_mask_idx, f"{col}_target_q75"] = te_arrays_q75[f"{col}_target_q75"]
                tr_rows.loc[real_mask_idx, f"{col}_target_cnt"] = te_arrays_cnt[f"{col}_target_cnt"]
            if col in STD_ENC_COLS:
                tr_rows.loc[real_mask_idx, f"{col}_target_std"] = te_arrays_std[f"{col}_target_std"]

        # Final TE feature columns checklist
        te_features = []
        for col in all_te_cols:
...
g_lb   = (c_mae * g_mae + c_rmse * g_rmse) * (1.0 + c_ev * (1.0 - g_ev)) + c_int

print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v703_hub_clean_te - v703 with Huber & OOF TE, no denoising)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v703_hub_clean_te.csv", index=False)
fold_report.to_csv("submissions/fold_report_v703_hub_clean_te.csv", index=False)
...
submission.to_csv("submission_v703_hub_clean_te.csv", index=False)
submission.to_csv("submissions/submission_v703_hub_clean_te.csv", index=False)
print(f"[DONE] Saved submission_v703_hub_clean_te.csv ({len(submission)} rows)")

np.save("oof_v703_hub_clean_te.npy", all_oof_stacked)
np.save("submissions/oof_v703_hub_clean_te.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v703_hub_clean_te.npy")

# -----------------------------------------------------------------
# 8. POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v703_hub_clean_te)")
print("=" * 75)
...
opt_lb   = (c_mae * opt_mae + c_rmse * opt_rmse) * (1.0 + c_ev * (1.0 - opt_ev)) + c_int

print(f"\nOptimized OOF LB Score: {opt_lb:.5f}")
print(f"  MAE: {opt_mae:.5f}, RMSE: {opt_rmse:.5f}, EV: {opt_ev:.5f}")

np.save("oof_v703_hub_clean_te_optimized.npy", opt_oof)
np.save("submissions/oof_v703_hub_clean_te_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v703_hub_clean_te_optimized.npy")
...
submission_opt.to_csv("submission_v703_hub_clean_te_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v703_hub_clean_te_optimized.csv", index=False)
print(f"[DONE] Saved submission_v703_hub_clean_te_optimized.csv ({len(submission_opt)} rows)")
