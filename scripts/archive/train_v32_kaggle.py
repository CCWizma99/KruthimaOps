"""
ML Opsidian: Genesis v32 - Multi-Seed Monolithic Ensemble Pipeline
========================================================================
Features of v32:
1. Bug 1 Fix: Excluded lat_decimal_len and lon_decimal_len from BASE_FEATURES.
2. Bug 2 Fix: Removed grid_id from TARGET_ENC_COLS to ensure GroupKFold OOF consistency.
3. Bug 3 Fix: Removed positive=True constraint on Ridge level-2 stackers for error correction.
4. Upgrade 1: Wrapped fold loop in a 4-seed sweep (seeds 42, 123, 456, 789) and averaged meta-features.
5. Upgrade 2: Implemented a grid search for Ridge alpha parameter in Level-2 stacking.
6. Upgrade 3: Applied 5x sample weight on real rows and 1x on synthetic rows during L1 training.
7. Upgrade 4: Turned off pseudo-labeling by default (USE_PSEUDO = False) to establish a clean baseline,
   with the code set up to use extreme confidence windows (<= 0.35 or >= 0.60) if enabled.
8. Output Paths: Saves results to submissions/ folder for evaluate.py.
"""
...
# CONFIGURATION
# -----------------------------------------------------------------
USE_PSEUDO = False  # Set to True to enable pseudo-labeling
PSEUDO_WINDOW_LOW = 0.35
PSEUDO_WINDOW_HIGH = 0.60

print("=" * 70)
print("  ML OPSIDIAN v32 - MULTI-SEED 7-MODEL ENSEMBLE PIPELINE")
print("=" * 70)
...
# -----------------------------------------------------------------
# 1.6. SEMI-SUPERVISED PSEUDO-LABELING (Upgrade 4)
# -----------------------------------------------------------------
...
if USE_PSEUDO:
    ...
    if os.path.exists(pseudo_path):
        ...
        # Filter highly confident predictions (extreme windows <= 0.35 or >= 0.60)
...
# Bug 2 Fix: Removed grid_id from TARGET_ENC_COLS to ensure GroupKFold OOF consistency
TARGET_ENC_COLS = [
...
# Bug 1 Fix: Added lat_decimal_len and lon_decimal_len to IGNORE_COLS
...
# Initialize Out-Of-Fold predictions lists (only evaluated on real rows)
real_mask = train_df['is_pseudo'] == 0

# Upgrade 1: Multi-seed averaging (seeds 42, 123, 456, 789)
SEEDS = [42, 123, 456, 789]
all_oof_meta = []
all_tst_meta = []

print("\n" + "=" * 70)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED ENSEMBLE (v32)")
print("=" * 70)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        # Upgrade 3: Build sample weights inside the fold loop
        ...
        # === 2. XGBoost with RMSE objective ===
        xgb_rmse = xgb.XGBRegressor(
            n_estimators=3000, learning_rate=0.05, max_depth=7,
        ...
        xgb_rmse.fit(X_tr_xgb, y_tr, sample_weight=sample_weights_tr, eval_set=[(X_va_xgb, y_va)], verbose=False)
        oof_preds["XGB-RMSE (d7)"][va_idx_clean] = xgb_rmse.predict(X_va_xgb)
        tst_preds["XGB-RMSE (d7)"] += xgb_rmse.predict(X_te_xgb) / N_FOLDS
        ...
        # === 7. LightGBM with RMSE loss ===
        lgb_rmse = lgb.LGBMRegressor(
            n_estimators=5000,
            learning_rate=0.03,
            num_leaves=15,
            max_depth=5,
        ...
        oof_preds["LGB-RMSE (d5)"][va_idx_clean] = lgb_rmse.predict(X_va)
        tst_preds["LGB-RMSE (d5)"] += lgb_rmse.predict(X_te) / N_FOLDS
        ...
        print(f"      Fold {fold+1}/{N_FOLDS} | XGB_M_it={xgb_mae.best_iteration:<4} XGB_R_it={xgb_rmse.best_iteration:<4} CAT_M1_it={cat_mae1.best_iteration_:<4} CAT_M2_it={cat_mae2.best_iteration_:<4} CAT_R_it={cat_rmse.best_iteration_:<4} LGB_M_it={lgb_mae.best_iteration_:<4} LGB_R_it={lgb_rmse.best_iteration_:<4} | [ENS MAE={f_mae:.4f}] [{time.time() - t0:.0f}s]")

    all_oof_meta.append(np.column_stack([oof_preds[m][real_mask] for m in MODEL_NAMES]))
    all_tst_meta.append(np.column_stack([tst_preds[m] for m in MODEL_NAMES]))

# Average Level-1 Out-Of-Fold and Test predictions across all seeds
oof_meta = np.mean(all_oof_meta, axis=0)
tst_meta = np.mean(all_tst_meta, axis=0)

# -----------------------------------------------------------------
# 6. RIDGE STACKING (Leak-Free Nested CV Stacking with Alpha Search)
# -----------------------------------------------------------------
print("\n" + "=" * 70)
print("  LEVEL-2: Leak-Free Nested CV Stacking (Ridge)")
print("=" * 70)
...
# Upgrade 2 & Bug 3 Fix: Ridge alpha parameter sweep (without positive constraint)
best_alpha, best_score = 1.0, np.inf
for alpha in [0.01, 0.1, 1.0, 10.0, 100.0]:
    ridge_cv = Ridge(alpha=alpha, fit_intercept=True)
    scores = cross_val_score(
        ridge_cv, oof_meta, original_y,
...
print(f"   [RIDGE] Best alpha: {best_alpha}  (OOF MAE: {best_score:.5f})")

# Fit stacking models on each fold split using the selected best_alpha
for fold, (tr_idx, va_idx) in enumerate(gkf_l2.split(original_df, original_y, original_groups)):
    stacker = Ridge(alpha=best_alpha, fit_intercept=True)
...
# Stacker coefficients for final full training fit
final_stacker = Ridge(alpha=best_alpha, fit_intercept=True)
...
c_mae, c_rmse, c_ev = 0.645758, 0.535669, 0.612893
...
oof_organic = oof_stacked[real_organic_mask]
...
print("\n" + "=" * 70)
print("  GLOBAL OOF RESULTS (v32 - Champion Monolithic Ensemble)")
print("=" * 70)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v32.csv", index=False)
fold_report.to_csv("submissions/fold_report_v32.csv", index=False)
...
submission.to_csv("submissions/submission_v32.csv", index=False)
print(f"[DONE] Saved submissions ({len(submission)} rows)")

# Save OOF (oof_stacked has exactly 20881 rows corresponding to train.csv)
np.save("oof_v32.npy", oof_stacked)
np.save("submissions/oof_v32.npy", oof_stacked)
print(f"[DONE] Saved oof_v32.npy and submissions/oof_v32.npy (for evaluate.py)")
