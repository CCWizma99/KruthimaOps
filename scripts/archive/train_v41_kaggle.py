"""
ML Opsidian: Genesis v41 - Upgraded Model Diversity Pipeline
==========================================================
Features of v41:
1. Multi-Seed Loop: Single seed [42] for speed.
2. Coordinate Precision Fingerprints Restored:
   - lat_decimal_len and lon_decimal_len are active features in BASE_FEATURES.
3. Downstream Signal Amplification:
   - confirmed_severe_risk
   - no_flood_confirmed
   - inundation_per_capita
   - downstream_risk_count (treated as categorical + target enc)
4. Model Ensemble Matrix Upgrade (v41):
   - XGB-MAE-1 (d7) with max_delta_step=1
   - CAT-MAE-1 (d7) - Deeper CatBoost for dominant estimator (depth=7 instead of 5)
   - CAT-MAE-2 (d5)
   - CAT-RMSE (d5)
   - LGB-MAE (d5)
...
print("=" * 75)
print("  ML OPSIDIAN v41 - MODEL DIVERSITY & EXPRESSIVENESS UPGRADES")
print("=" * 75)
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v41 PIPELINE")
print("=" * 75)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        # === 2. CatBoost-MAE-1 (d7) - Deeper CatBoost ===
        ...
        oof_preds["CAT-MAE-1 (d7)"][va_idx_clean] = cat_mae1.predict(X_va_cat)
        tst_preds["CAT-MAE-1 (d7)"] += cat_mae1.predict(X_te_cat) / N_FOLDS
        ...
        # === 6. XGBoost-MAE-2 (d5) - Replaces LGB-DART ===
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v41 - Raw Custom Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v41.csv", index=False)
fold_report.to_csv("submissions/fold_report_v41.csv", index=False)
...
submission.to_csv("submissions/submission_v41.csv", index=False)
print(f"[DONE] Saved submission_v41.csv ({len(submission)} rows)")

np.save("oof_v41.npy", all_oof_stacked)
np.save("submissions/oof_v41.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v41.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v41)")
print("=" * 75)
...
submission_opt.to_csv("submission_v41_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v41_optimized.csv", index=False)
print(f"[DONE] Saved submission_v41_optimized.csv ({len(submission_opt)} rows)")
