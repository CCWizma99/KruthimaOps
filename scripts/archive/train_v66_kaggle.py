"""
ML Opsidian: Genesis v66 - Upgraded Stacking & Pseudo-Labeling Pipeline
========================================================================
Refinements over v42:
1. Upgraded Pseudo-Label Source:
   - Upgrades soft pseudo-labels source from submission_v30.csv (0.38293)
     to submission_v42.csv (the raw, uncalibrated v42 predictions).
...
print("=" * 75)
print("  ML OPSIDIAN v66 - UPGRADED GEOSPATIAL & TRANSDUCTIVE PIPELINE")
print("=" * 75)
...
if USE_PSEUDO:
    # Upgrade pseudo label source to submission_v42.csv (raw uncalibrated v42 predictions)
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - SINGLE-SEED v66 PIPELINE")
print("=" * 75)
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v66 - Raw Custom Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v66.csv", index=False)
fold_report.to_csv("submissions/fold_report_v66.csv", index=False)
...
submission.to_csv("submission_v66.csv", index=False)
submission.to_csv("submissions/submission_v66.csv", index=False)
print(f"[DONE] Saved submission_v66.csv ({len(submission)} rows)")

np.save("oof_v66.npy", all_oof_stacked)
np.save("submissions/oof_v66.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v66.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v66)")
print("=" * 75)
...
np.save("oof_v66_optimized.npy", opt_oof)
np.save("submissions/oof_v66_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v66_optimized.npy")
...
submission_opt.to_csv("submission_v66_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v66_optimized.csv", index=False)
print(f"[DONE] Saved submission_v66_optimized.csv ({len(submission_opt)} rows)")
