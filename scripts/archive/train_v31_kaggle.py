"""
ML Opsidian: Genesis v31 - Monolithic Champion Ensemble Pipeline
========================================================================
Features of v31:
1. Precision Fingerprints: Engineers lat_decimal_len and lon_decimal_len to isolate synthetics.
2. Pseudo-Labeling: Semi-supervised learning using the 0.38293 champion submission_v30.csv.
...
5. Level-1 Models (7-Model Diverse Ensemble):
   - XGB-MAE (d7) & XGB-RMSE (d7)
   - CAT-MAE-1 (d5), CAT-MAE-2 (d5), & CAT-RMSE (d5)
   - LGB-MAE (d5) & LGB-RMSE (d5)
6. Level-2 Meta-Learner: Stacking with regularized Ridge(alpha=1.0, positive=True) fit strictly on real rows.
7. Unweighted Training: Rejects sample weights to avoid geospatial imputation overfitting.
8. Output Paths: Saves results to both root and submissions/ folder for evaluate.py.
"""
...
# -----------------------------------------------------------------
print("=" * 70)
print("  ML OPSIDIAN v31 - MONOLITHIC 7-MODEL ENSEMBLE PIPELINE")
print("=" * 70)
print("\n[LOAD] Loading data...")
...
# -----------------------------------------------------------------
print("\n[SEMI-SUPERVISED] Pseudo-Labeling from submission_v30.csv...")
...
# Initialize Out-Of-Fold predictions dictionary (only evaluated on real rows)
...
print("\n" + "=" * 70)
print(f"  5-FOLD SPATIAL GROUP CV (SEED 42) - UNWEIGHTED v31")
print("=" * 70)
...
for fold, (tr_idx, va_idx) in enumerate(gkf_l2.split(original_df, original_y, original_groups)):
    # Regularized positive stacker
    stacker = Ridge(alpha=1.0, fit_intercept=True, positive=True)
...
final_stacker = Ridge(alpha=1.0, fit_intercept=True, positive=True)
...
g_lb   = (0.645758 * g_mae + 0.535669 * g_rmse) * (1.0 + 0.612893 * (1.0 - g_ev))
...
r_lb   = (0.645758 * r_mae + 0.535669 * r_rmse) * (1.0 + 0.612893 * (1.0 - r_ev))

print("\n" + "=" * 70)
print("  GLOBAL OOF RESULTS (v31 - Champion Monolithic Ensemble)")
print("=" * 70)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v31.csv", index=False)
fold_report.to_csv("submissions/fold_report_v31.csv", index=False)
...
submission.to_csv("submission_v31.csv", index=False)
submission.to_csv("submissions/submission_v31.csv", index=False)
print(f"[DONE] Saved submissions ({len(submission)} rows)")

# Save OOF (oof_stacked has exactly 20881 rows corresponding to train.csv)
np.save("oof_v31.npy", oof_stacked)
np.save("submissions/oof_v31.npy", oof_stacked)
print(f"[DONE] Saved oof_v31.npy and submissions/oof_v31.npy (for evaluate.py)")
