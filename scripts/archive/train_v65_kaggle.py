"""
ML Opsidian: Genesis v65 - Crazy ML Scientist Stacking Pipeline
==================================================================================
Features of v65:
1. Smarter Conflict Key: Categoricals + 5-quantile buckets of coordinates, elevation, distance to river, and rainfall.
2. Separate Pseudo-Row Conflict Resolution: Prevents soft labels from contaminating real row targets.
3. Target Power Normalization (Yeo-Johnson) Inside Folds: Fit on resolved targets of tr_rows, transform validation target for early stopping, inverse transform back to raw space before stacking.
4. Pure Downstream Oracle: 7th base model (CatBoost) trained strictly on 4 downstream categorical/continuous features in raw space.
5. 4-Seed Stacking + Structural Diversity: SEEDS = [42, 2024, 7, 999], where seed 999 uses a heavier regularization profile.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v65 - CRAZY ML SCIENTIST STACKING PIPELINE")
print("=" * 75)
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v65 PIPELINE")
print("=" * 75)
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v65 - Raw Custom Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v65.csv", index=False)
fold_report.to_csv("submissions/fold_report_v65.csv", index=False)
...
submission.to_csv("submissions/submission_v65.csv", index=False)
print(f"[DONE] Saved submission_v65.csv ({len(submission)} rows)")

np.save("oof_v65.npy", all_oof_stacked)
np.save("submissions/oof_v65.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v65.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v65)")
print("=" * 75)
...
np.save("oof_v65_optimized.npy", opt_oof)
np.save("submissions/oof_v65_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v65_optimized.npy")
...
submission_opt.to_csv("submission_v65_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v65_optimized.csv", index=False)
print(f"[DONE] Saved submission_v65_optimized.csv ({len(submission_opt)} rows)")
