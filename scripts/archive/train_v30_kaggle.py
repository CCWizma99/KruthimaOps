"""
ML Opsidian: Genesis v30 - Precision Fingerprints, SLSQP Pseudo-Labels & GroupKFold CV
========================================================================
Features of v30:
...
5. Level-1 Models: XGB-MAE, 2x CAT-MAE, CAT-RMSE, and LGB-MAE.
6. Level-2 Meta-Learner: Nested CV stacking with LinearRegression(positive=True) fit on real rows.
...
# -----------------------------------------------------------------
print("=" * 70)
print("  ML OPSIDIAN v30 - PRECISION FINGERPRINTS & SLSQP PSEUDO-LABELS")
print("=" * 70)
print("\n[LOAD] Loading data...")
...
print("\n" + "=" * 70)
print(f"  5-FOLD SPATIAL GROUP CV (SEED 42) - UNWEIGHTED v30")
print("=" * 70)
...
print("\n" + "=" * 70)
print("  GLOBAL OOF RESULTS (v30 - Precision Fingerprints & SLSQP Pseudo-Labels)")
print("=" * 70)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v30.csv", index=False)
fold_report.to_csv("submissions/fold_report_v30.csv", index=False)
...
submission.to_csv("submission_v30.csv", index=False)
submission.to_csv("submissions/submission_v30.csv", index=False)
print(f"[DONE] Saved submissions ({len(submission)} rows)")

# Save OOF (oof_stacked has exactly 20881 rows corresponding to train.csv)
np.save("oof_v30.npy", oof_stacked)
np.save("submissions/oof_v30.npy", oof_stacked)
print(f"[DONE] Saved oof_v30.npy and submissions/oof_v30.npy (for evaluate.py)")
