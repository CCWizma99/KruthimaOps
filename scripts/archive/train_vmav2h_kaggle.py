"""
ML Opsidian: Genesis vmav2h - Adversarial Drop v2 + CAT-HUBER (5 SEEDS)
=======================================================================
Base: vmav2h_s1 (Single-Seed Probe)
Upgraded to:
  - 5 Seeds: SEEDS = [42, 2024, 7, 100, 999]
  - Saved outputs: vmav2h
"""
...
print("=" * 75)
print("  ML OPSIDIAN vmav2h - ADVERSARIAL DROP v2 + CAT-HUBER (5 SEEDS)")
print("=" * 75)
...
def engineer_features(df):
    ...
    # [ADVERSARIAL DROP v2] urban_runoff_potential removed
    ...
    # [ADVERSARIAL DROP v2] drainage_deficit removed
    # [ADVERSARIAL DROP v2] infra_resilience removed
    ...
    # [ADVERSARIAL DROP v2] terrain_roughness_index removed (drops terrain_veg_risk too)
    df["is_repeat_flood_zone"]         = (df["historical_flood_count"] > 2).astype(int)
    # [ADVERSARIAL DROP v2] rain_spike_ratio removed
    df["confirmed_risk"]               = (
        (df["flood_occurrence_current_event"].astype(str).str.strip().str.lower() == "yes") &
        (df["is_good_to_live"].astype(str).str.strip().str.lower() == "no")
    ).astype(int)
    ...
    # [ADVERSARIAL DROP v2] soil_drainage_saturation removed
...
# [ADVERSARIAL DROP v2] Drop list extended with the 8 features
IGNORE_COLS = DROP_COLS + [
    ...
    "infra_resilience", "urban_runoff_potential", "terrain_veg_risk"
]
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED vmav2h PIPELINE")
print("=" * 75)
...
for seed in SEEDS:
    print(f"\n==================== RUNNING SEED {seed} ====================")
    y_full = train_df[TARGET_RAW].values
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y_full, groups)):
        ...
        group_medians_tr = tr_rows.groupby([temp_tr[c] for c in temp_tr.columns])[TARGET_RAW].transform('median')
        tr_rows[TARGET_RAW] = group_medians_tr
        ...
        y_tr, y_va = tr_rows[TARGET_RAW], va_rows[TARGET_RAW].values
    ...
    oof_meta_seed = np.column_stack([oof_preds[m][real_mask_global] for m in MODEL_NAMES])
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (vmav2h - Adversarial Drop v2 + CAT-HUBER)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_vmav2h.csv", index=False)
fold_report.to_csv("submissions/fold_report_vmav2h.csv", index=False)
...
submission.to_csv("submission_vmav2h.csv", index=False)
submission.to_csv("submissions/submission_vmav2h.csv", index=False)
print(f"[DONE] Saved submission_vmav2h.csv ({len(submission)} rows)")

np.save("oof_vmav2h.npy", all_oof_stacked)
np.save("submissions/oof_vmav2h.npy", all_oof_stacked)
print(f"[DONE] Saved oof_vmav2h.npy")

# -----------------------------------------------------------------
# 8. POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (vmav2h)")
print("=" * 75)
...
np.save("oof_vmav2h_optimized.npy", opt_oof)
np.save("submissions/oof_vmav2h_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_vmav2h_optimized.npy")
...
submission_opt.to_csv("submission_vmav2h_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_vmav2h_optimized.csv", index=False)
print(f"[DONE] Saved submission_vmav2h_optimized.csv ({len(submission_opt)} rows)")
