"""
ML Opsidian: Genesis v703 - v70 + Value Frequency Count Features
==================================================================================
Base: v70 (6-Model, 3-Seed, Broad Conflict-Key Stacking Pipeline)
Exactly 1 change vs v70:

Value Frequency Count Features (from ai-says.txt suggestion):
  Synthetic data generators produce values at discrete grid points — certain
  float values repeat many times (grid nodes) while others are unique.
  Mapping each raw continuous value's frequency back as a standalone column
  exposes hidden sampling thresholds to the tree models.

  Computed on combined (train + test) raw data BEFORE imputation, so the
  frequency map reflects the true synthetic generation grid density.
  Applied to: latitude, longitude, elevation_m, distance_to_river_m,
              rainfall_7d_mm, monthly_rainfall_mm, inundation_area_sqm.

Everything else is identical to v70 (same pseudo-labels, same conflict
resolution, same 6 models, same 3 seeds, same post-hoc bounds).
"""
...
print("=" * 75)
print("  ML OPSIDIAN v1400 - FRANKENSTEIN ARCHITECTURE (MAD SCIENCE)")
print("=" * 75)
...
# -----------------------------------------------------------------
# 1.4. [NEW] VALUE FREQUENCY COUNT FEATURES
# Compute on raw combined data BEFORE imputation to capture true
# synthetic grid density. Each value's frequency = how many times
# that exact float appears across train+test combined.
# -----------------------------------------------------------------
...
print("\n[MAD SCIENCE] Discovering 150 Topological Anchor Points via KMeans...")
coords = combined[['latitude', 'longitude']].dropna()
kmeans = MiniBatchKMeans(n_clusters=150, random_state=42, batch_size=1024, n_init=3)
kmeans.fit(coords)
anchors = kmeans.cluster_centers_

print("\n[MAD SCIENCE] Building Directed Graph Hydrology (PageRank)...")
tree = cKDTree(coords.values)
pairs = tree.query_pairs(r=0.05) # ~5.5km radius
G = nx.DiGraph()
elevations = combined['elevation_m'].values
G.add_nodes_from(range(len(combined)))

edges_to_add = []
for i, j in pairs:
    if elevations[i] > elevations[j]:
        edges_to_add.append((i, j))
    elif elevations[j] > elevations[i]:
        edges_to_add.append((j, i))

G.add_edges_from(edges_to_add)
pagerank_scores = nx.pagerank(G, alpha=0.85)
combined['pagerank_water_sink'] = [pagerank_scores.get(i, 0.0) for i in range(len(combined))]

train_df['pagerank_water_sink'] = combined['pagerank_water_sink'].values[:len(train_df)]
test_df['pagerank_water_sink'] = combined['pagerank_water_sink'].values[len(train_df):]
...
def engineer_features(df):
    ...
    for i, anchor in enumerate(anchors):
        df[f'dist_to_anchor_{i}'] = haversine_vectorized(lat_val, lon_val, anchor[0], anchor[1])
...
# -----------------------------------------------------------------
# 5. MODEL DEFINITIONS (identical to v70)
# -----------------------------------------------------------------
MODEL_NAMES = [
    "XGB-MAE-1 (d7)",
    "CAT-MAE-1 (d5)",
...
N_FOLDS     = 5
original_y_clean = train_df[TARGET].copy()

print("\n[MAD SCIENCE] Poisoning target variables with Gaussian Noise (Brutal Regularization)...")
np.random.seed(42)
poisoned_y = original_y_clean + np.random.normal(0, 0.005, size=len(original_y_clean))
poisoned_y = np.clip(poisoned_y, 0.0, 1.0)
train_df[TARGET] = poisoned_y
...
original_y = original_y_clean.values[real_mask]
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - SINGLE-SEED v1400 PIPELINE")
print("=" * 75)
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v1400 - Frankenstein Architecture)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v1400.csv", index=False)
fold_report.to_csv("submissions/fold_report_v1400.csv", index=False)
...
submission.to_csv("submission_v1400.csv", index=False)
submission.to_csv("submissions/submission_v1400.csv", index=False)
print(f"[DONE] Saved submission_v1400.csv ({len(submission)} rows)")

np.save("oof_v1400.npy", all_oof_stacked)
np.save("submissions/oof_v1400.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v1400.npy")

# -----------------------------------------------------------------
# 8. POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v1400)")
print("=" * 75)
...
np.save("oof_v1400_optimized.npy", opt_oof)
np.save("submissions/oof_v1400_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v1400_optimized.npy")

opt_test_preds = a_opt * np.power(np.clip(tst_stacked_avg, 1e-6, None), b_opt) + c_opt
opt_test_preds = np.clip(opt_test_preds, 0.0, 1.0)

print("\n[MAD SCIENCE] Histogram Forcing (Quantile Mapping)...")
y_train_sorted = np.sort(original_y)
percentiles = np.linspace(0, 100, len(opt_test_preds))
forced_values = np.percentile(y_train_sorted, percentiles)
sort_idx = np.argsort(opt_test_preds)
forced_test_preds = np.zeros_like(opt_test_preds)
forced_test_preds[sort_idx] = forced_values
opt_test_preds = forced_test_preds

submission_opt = pd.DataFrame({
    "record_id"       : test_df[ID_COL],
    "flood_risk_score": opt_test_preds
})
submission_opt.to_csv("submission_v1400_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v1400_optimized.csv", index=False)
print(f"[DONE] Saved submission_v1400_optimized.csv ({len(submission_opt)} rows)")
