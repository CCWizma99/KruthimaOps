"""
ML Opsidian: Genesis v35 - The Symmetry Engine Pipeline (Single-Seed)
========================================================================
Features of v35:
1. Clean Rollback: Unweighted training (1.0 weight) to avoid geospatial imputation overfitting.
2. Precision Fingerprints: Engineers lat_decimal_len and lon_decimal_len to isolate synthetics.
3. Center Pseudo-Labeling: Restored from v30 with the 0.46-0.49 confidence window using submission_v30.csv.
4. Loss Symmetry Model Matrix:
   - XGB-Custom (d7): XGBoost running our custom joint objective calibrated to our simulator (alpha=0.392696, beta=0.875527).
   - CAT-MAE-1 (d5) & CAT-MAE-2 (d5): Targets absolute error (medians).
   - CAT-RMSE (d5): Targets squared error (means).
   - LGB-MAE (d5): Targets absolute error.
5. Level-2 Stacking: Single-seed leak-free GroupKFold nested CV Ridge stacking.
...
print("=" * 70)
print("  ML OPSIDIAN v35 - THE SYMMETRY ENGINE PIPELINE")
print("=" * 70)
...
# -----------------------------------------------------------------
# 4.3. CUSTOM METRIC-DRIVEN LOSS FOR XGBOOST
# -----------------------------------------------------------------
def joint_mae_rmse_objective(y_true, y_pred):
    """
    Custom objective for XGBoost. 
    Calibrated to exact simulator formula:
    Base Error = (alpha * MAE + beta * RMSE)
    where alpha = 0.392696, beta = 0.875527
    """
    residual = y_pred - y_true
    
    alpha = 0.392696  # MAE Weight
    beta = 0.875527   # RMSE Weight
    delta = 1e-3      # Pseudo-Huber smoothing for L1 gradient
...
print("\n" + "=" * 70)
print(f"  5-FOLD SPATIAL GROUP CV - SINGLE-SEED SYMMETRY ENGINE ENSEMBLE (v35)")
print("=" * 70)
...
for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
    ...
    xgb_custom.fit(X_tr_xgb, y_tr, eval_set=[(X_va_xgb, y_va)], verbose=False)
    oof_preds["XGB-Custom (d7)"][va_idx_clean] = xgb_custom.predict(X_va_xgb)
    tst_preds["XGB-Custom (d7)"] += xgb_custom.predict(X_te_xgb) / N_FOLDS
    ...
    print(f"      Fold {fold+1}/{N_FOLDS} | XGB_C_it={xgb_custom.best_iteration:<4} CAT_M1_it={cat_mae1.best_iteration_:<4} CAT_M2_it={cat_mae2.best_iteration_:<4} CAT_R_it={cat_rmse.best_iteration_:<4} LGB_M_it={lgb_mae.best_iteration_:<4} | [ENS MAE={f_mae:.4f}] [{time.time() - t0:.0f}s]")

# -----------------------------------------------------------------
# 6. RIDGE STACKING (Leak-Free Nested CV)
# -----------------------------------------------------------------
...
print(f"\n   [RIDGE] Best alpha: {best_alpha}  (OOF MAE: {best_score:.5f})")

# 2. Leak-Free Level-2 OOF stacking (GroupKFold)
...
for fold, (tr_idx, va_idx) in enumerate(gkf_l2.split(original_df, original_y, original_groups)):
    stacker = Ridge(alpha=best_alpha, fit_intercept=True, positive=True)
    stacker.fit(oof_meta[tr_idx], original_y[tr_idx])
    all_oof_stacked[va_idx] = stacker.predict(oof_meta[va_idx])
    
# 3. Fit final stacker on full meta-features to predict test
final_stacker = Ridge(alpha=best_alpha, fit_intercept=True, positive=True)
final_stacker.fit(oof_meta, original_y)
tst_stacked = final_stacker.predict(tst_meta)
...
# -----------------------------------------------------------------
# Newly calibrated simulator coefficients (optimized over all 10 historical points)
...
print("\n" + "=" * 70)
print("  GLOBAL OOF RESULTS (v35 - Single-Seed Symmetry Engine)")
print("=" * 70)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v35.csv", index=False)
fold_report.to_csv("submissions/fold_report_v35.csv", index=False)
...
submission.to_csv("submission_v35.csv", index=False)
submission.to_csv("submissions/submission_v35.csv", index=False)
...
np.save("oof_v35.npy", all_oof_stacked)
np.save("submissions/oof_v35.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v35.npy and submissions/oof_v35.npy (for evaluate.py)")
