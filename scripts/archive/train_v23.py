"""
ML Opsidian: Genesis v23 - Median Align, Multi-Seed & Interaction Synergy
========================================================================
...
# -----------------------------------------------------------------
print("=" * 70)
print("  ML OPSIDIAN v23 - TARGETED 7-POINT UPGRADE")
print("=" * 70)
print("\n[LOAD] Loading data...")
...
if os.path.exists("submissions/submission_v20.csv"):
    ...
    # Filter highly confident predictions (extremes)
...
def engineer_features(df):
    ...
    # 3. Composite Interaction Features (Granular Geography)
    df['district_flood_key'] = df['district'].astype(str) + "_" + df['flood_occurrence_current_event'].astype(str).str.strip()
    df['district_livable_key'] = df['district'].astype(str) + "_" + df['is_good_to_live'].astype(str).str.strip()
...
COMPOSITE_ENC_COLS = [
    "district_flood_key", "district_livable_key"
]
...
IGNORE_COLS = DROP_COLS + [TARGET, "flood_occurrence_yes", "lat_decimal_len", "lon_decimal_len",
                           "downstream_sig", "infra_deficit_sig", "district_flood_key", "district_livable_key"]
...
for seed_idx, seed in enumerate(SEEDS):
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        cat_mae1 = cb.CatBoostRegressor(
            ...
            max_ctr_complexity=2, # 4. Internal Interactions
...
print("\n" + "=" * 70)
print("  GLOBAL MULTI-SEED RESULTS (v23 - Targeted 7-Point Upgrade)")
print("=" * 70)
...
fold_report.to_csv("submissions/fold_report_v23.csv", index=False)
print(f"\n[DONE] Saved fold_report_v23.csv")
...
submission.to_csv("submissions/submission_v23.csv", index=False)
print(f"[DONE] Saved submission_v23.csv ({len(submission)} rows)")

np.save("submissions/oof_v23.npy", oof_predictions_accum[real_mask])
print(f"[DONE] Saved oof_v23.npy (for evaluate.py)")
