"""
ML Opsidian: Genesis v50 - The Robust Hybrid Stacking Pipeline
==============================================================
Upgrades and Bug Fixes from v46/v47/v49:
1. Rollback of Failed v49 Experiments:
   - Restored unconstrained XGB-MAE-1 (d7) and XGB-MAE-2 (d5).
   - Removed Monotonic Constraints and Geospatial Kriging which degraded tabular synthetic target modeling.
   - Removed low-weight KNN-RealRow model.
2. Leak-Free Leave-One-Out (LOO) Target Encoding:
   - Built inside the CV loop for 'downstream_sig' and 'district'.
   - Excludes pseudo-labeled test rows from stats calculations to prevent test-to-train target leaks.
3. Quantile Ensemble Base Model:
   - Trains 5 CatBoost quantile regressors at [0.40, 0.45, 0.50, 0.55, 0.60].
   - Feeds their median prediction as the 7th base model into the Ridge stacker.
4. Bug-Fixed Fallback-Enabled Per-Group Calibration:
   - Fits separate power transformations (a * pred^b + c) for downstream_sig risk groups.
   - Falls back to global parameters in sparse/unseen groups.
5. Bug-Fixed Analytical Group Median Blend:
   - Computes Bayesian-smoothed group medians using ONLY real training rows (fixed leak in v46/v47).
   - Blends analytical medians with stacked predictions, optimized on OOF CV. Toggleable via `USE_BLEND`.
6. Configurable Adversarial Validation Weighting:
   - Optionally weights real training rows based on their "test-likeness" in coordinate/climate space to correct spatial covariate shift. Toggleable via `USE_ADVERSARIAL_WEIGHTS`.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v50 - THE ROBUST HYBRID STACKING PIPELINE")
print("=" * 75)
...
if USE_PSEUDO:
    ...
    if os.path.exists(pseudo_path):
        ...
        fallback_path = "submissions/submission_v30.csv"
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - SINGLE-SEED v50 PIPELINE")
print("=" * 75)
...
for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
    ...
    # --- Compute CV-Safe Analytical Group Medians (Bug Fixed: real rows only) ---
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v50 - Raw Stacking)")
print("=" * 75)
...
fold_report.to_csv("fold_report_v50.csv", index=False)
fold_report.to_csv("submissions/fold_report_v50.csv", index=False)
...
submission.to_csv("submission_v50.csv", index=False)
submission.to_csv("submissions/submission_v50.csv", index=False)
print(f"[DONE] Saved submission_v50.csv ({len(submission)} rows)")

np.save("oof_v50.npy", oof_stacked)
np.save("submissions/oof_v50.npy", oof_stacked)
print(f"[DONE] Saved oof_v50.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED FALLBACK-ENABLED PER-GROUP POWER TRANSFORMATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v50)")
print("=" * 75)
...
if USE_BLEND:
    print("\n" + "=" * 75)
    print("  DYNAMIC ANALYTICAL GROUP MEDIAN BLEND (v50)")
    print("=" * 75)
...
np.save("oof_v50_optimized.npy", final_oof)
np.save("submissions/oof_v50_optimized.npy", final_oof)
print(f"[DONE] Saved oof_v50_optimized.npy")
...
submission_opt.to_csv("submission_v50_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v50_optimized.csv", index=False)
print(f"[DONE] Saved submission_v50_optimized.csv ({len(submission_opt)} rows)")
