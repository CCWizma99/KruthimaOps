"""
ML Opsidian: Genesis v61 - Leak-Free Conflict Resolution, Honest Validation & Fast Convergence
==============================================================================
Upgrades from v58:
1. Increased learning rates (0.03 -> 0.1/0.12) to speed up model convergence.
2. Decreased max iterations/estimators to the 800–1000 range.
3. Decreased early stopping patience to 40–50 rounds to ensure models stop in the hundreds of iterations.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v61 - LEAK-FREE CONFLICT RESOLUTION, HONEST VALIDATION & FAST CONVERGENCE")
print("=" * 75)
...
# -----------------------------------------------------------------
# 1.6. SEMI-SUPERVISED PSEUDO-LABELING (v61 priority)
# -----------------------------------------------------------------
...
if USE_PSEUDO:
    # Prioritized highest quality submissions first
    pseudo_paths_to_try = [
        "submission_v44_optimized.csv", "submissions/submission_v44_optimized.csv",
        "submission_v42_optimized.csv", "submissions/submission_v42_optimized.csv",
        "submission_v45_optimized.csv", "submissions/submission_v45_optimized.csv",
        "submission_v48_optimized.csv", "submissions/submission_v48_optimized.csv",
        "submission_v49_optimized.csv", "submissions/submission_v49_optimized.csv",
...
def engineer_features(df):
    ...
    # Inundation quantile bucket feature
...
downstream_cols = [
    ...
    "downstream_risk_count", "downstream_sig", "downstream_quad_sig", "confirmed_severe_risk", "no_flood_confirmed",
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - SINGLE-SEED v61 PIPELINE (FAST CONVERGENCE)")
print("=" * 75)
...
# -----------------------------------------------------------------
# 5.5. CUSTOM EV-MAE OBJECTIVE FUNCTION
# -----------------------------------------------------------------
...
for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
    ...
    # Apply conflict resolution ONLY on the training fold (tr_rows)
    ...
    # Compute Hierarchical Spatial Encoding Residuals
    for tgt_df in [tr_rows, va_rows, test_df]:
    ...
    # Drop raw fine-grained spatial encodings
    grid_cols_to_drop = [
    ...
    # --- Base Level-1 Model Training (Fast Convergence Tuning: 100s of iterations) ---

    # 1. XGB-EV-MAE-1 (d7) - Custom objective function
    xgb_m1 = xgb.XGBRegressor(
        n_estimators=1000, learning_rate=0.1, max_depth=7, min_child_weight=4,
        ...
        device=XGB_DEVICE,
        enable_categorical=False, early_stopping_rounds=50, random_state=SEED, n_jobs=-1
    )
    xgb_m1.fit(X_tr_xgb, y_tr, sample_weight=w_tr, eval_set=[(X_va_xgb, y_va)], verbose=False)
    
    # 2. CAT-MAE-1 (d5)
    cat_m1 = cb.CatBoostRegressor(
        iterations=1000, learning_rate=0.12, depth=5, l2_leaf_reg=5.0,
    ...
    cat_m1.fit(X_tr_cat, y_tr, sample_weight=w_tr, cat_features=cat_cols, eval_set=(X_va_cat, y_va), early_stopping_rounds=50, verbose=False)

    # 3. CAT-MAE-2 (d5)
    cat_m2 = cb.CatBoostRegressor(
        iterations=1000, learning_rate=0.12, depth=5, l2_leaf_reg=12.0,
    ...
    cat_rmse = cb.CatBoostRegressor(
        iterations=1000, learning_rate=0.12, depth=5, l2_leaf_reg=8.0,
    ...
    cat_rmse.fit(X_tr_cat, y_tr, sample_weight=w_tr, cat_features=cat_cols, eval_set=(X_va_cat, y_va), early_stopping_rounds=50, verbose=False)

    # 5. LGB-MAE (d5)
    lgb_m1 = lgb.LGBMRegressor(
        n_estimators=1000, learning_rate=0.1, num_leaves=31, max_depth=5,
    ...
    lgb_m1.fit(X_tr, y_tr, sample_weight=w_tr, eval_set=[(X_va, y_va)], callbacks=[lgb.early_stopping(50, verbose=False)])

    # 6. XGB-EV-MAE-2 (d5) - Custom objective function
    xgb_m2 = xgb.XGBRegressor(
        n_estimators=1000, learning_rate=0.1, max_depth=5, min_child_weight=6,
    ...
    # 7. Quantile Ensemble Base Model (Averaging 5 alphas)
    quantile_preds_va = []
    quantile_preds_te = []
    for alpha in QUANTILE_ALPHAS:
        qmodel = cb.CatBoostRegressor(
            iterations=800, learning_rate=0.12, depth=5, l2_leaf_reg=5.0,
        ...
        qmodel.fit(X_tr_cat, y_tr, sample_weight=w_tr, cat_features=cat_cols, eval_set=(X_va_cat, y_va), early_stopping_rounds=40, verbose=False)
    ...
    cat_rg.fit(X_tr_cat, y_tr_trans, sample_weight=w_tr, cat_features=cat_cols, eval_set=(X_va_cat, y_va_trans), early_stopping_rounds=50, verbose=False)
    ...
    q_arr_va = np.array(quantile_preds_va)
    q_arr_te = np.array(quantile_preds_te)
...
print(f"      {'Quantile IQR':<18}: {w_final[-2]:.4f}")
print(f"      {'Quantile Skew':<18}: {w_final[-1]:.4f}")
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v61 - Raw Stacking)")
print("=" * 75)
...
fold_report.to_csv("fold_report_v61.csv", index=False)
fold_report.to_csv("submissions/fold_report_v61.csv", index=False)
...
submission.to_csv("submissions/submission_v61.csv", index=False)
print(f"[DONE] Saved submission_v61.csv ({len(submission)} rows)")

np.save("oof_v61.npy", oof_stacked)
np.save("submissions/oof_v61.npy", oof_stacked)
print(f"[DONE] Saved oof_v61.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED FALLBACK-ENABLED PER-GROUP POWER TRANSFORMATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v61)")
print("=" * 75)
...
if USE_BLEND:
    print("\n" + "=" * 75)
    print("  DYNAMIC ANALYTICAL GROUP MEDIAN BLEND (v61)")
    print("=" * 75)
...
np.save("oof_v61_optimized.npy", final_oof)
np.save("submissions/oof_v61_optimized.npy", final_oof)
print(f"[DONE] Saved oof_v61_optimized.npy")
...
submission_opt.to_csv("submission_v61_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v61_optimized.csv", index=False)
print(f"[DONE] Saved submission_v61_optimized.csv ({len(submission_opt)} rows)")
