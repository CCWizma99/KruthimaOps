"""
ML Opsidian: Genesis vc1 - Crazy 01 Hybrid Pipeline
==================================================================================
Features of vc1:
1. Target-Inversion Score (TIS): Hand-crafted physical index using inverted correlations.
2. Geospatial Risk Gradients (GRG): Micro-spatial derivatives based on multi-scale target encodings.
3. Smarter Conflict Key: Categoricals + 5-quantile buckets of coordinates, elevation, distance to river, and rainfall.
4. Separate Pseudo-Row Conflict Resolution: Prevents soft labels from contaminating real row targets.
5. Target Power Normalization (Yeo-Johnson) Inside Folds: Fit on resolved targets of tr_rows, transform validation target for early stopping, inverse transform back to raw space before stacking.
6. Pure Downstream Oracle: 7th base model (CatBoost) trained strictly on 4 downstream categorical/continuous features in raw space.
7. 4-Seed Stacking + Structural Diversity: SEEDS = [42, 2024, 7, 999], where seed 999 uses a heavier regularization profile.
"""
...
print("=" * 75)
print("  ML OPSIDIAN vc1 - CRAZY 01 HYBRID PIPELINE")
print("=" * 75)
...
def engineer_features(df):
    ...
    # [CRAZY 01 FEATURE] Target-Inversion Score (TIS)
    # Combining the top-correlated physical features in their true inverted directions
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED vc1 PIPELINE")
print("=" * 75)
...
for seed in SEEDS:
    ...
    # Heavier regularization profile for structural diversity on the 4th seed
    if seed == 999:
        print("   [Diversity] Enabling heavy regularization profile for Seed 999...")
        xgb1_alpha, xgb1_lambda, xgb1_gamma = 5.0, 10.0, 0.5
        cat1_l2 = 15.0
        cat2_l2 = 25.0
        cat_rmse_l2 = 20.0
        lgb_alpha, lgb_lambda = 5.0, 12.0
        xgb2_alpha, xgb2_lambda, xgb2_gamma = 10.0, 20.0, 1.0
    else:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        # [CRAZY 01 FEATURE] Compute Geospatial Risk Gradients (GRG)
        for tgt_df in [tr_rows, va_rows, test_df]:
        ...
        # Append geospatial risk gradients to active features
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (vc1 - Raw Custom Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_vc1.csv", index=False)
fold_report.to_csv("submissions/fold_report_vc1.csv", index=False)
...
submission.to_csv("submission_vc1.csv", index=False)
submission.to_csv("submissions/submission_vc1.csv", index=False)
print(f"[DONE] Saved submission_vc1.csv ({len(submission)} rows)")

np.save("oof_vc1.npy", all_oof_stacked)
np.save("submissions/oof_vc1.npy", all_oof_stacked)
print(f"[DONE] Saved oof_vc1.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (vc1)")
print("=" * 75)
...
np.save("oof_vc1_optimized.npy", opt_oof)
np.save("submissions/oof_vc1_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_vc1_optimized.npy")
...
submission_opt.to_csv("submission_vc1_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_vc1_optimized.csv", index=False)
print(f"[DONE] Saved submission_vc1_optimized.csv ({len(submission_opt)} rows)")
