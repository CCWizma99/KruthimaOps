...
"""
ML Opsidian: Genesis v19 - Metric-Informed Goldilocks
======================================================
REVERSE-ENGINEERED METRIC:
  LB = -22.87*MAE + 6.60*RMSE + 3.03*(1-EV)

KEY INSIGHT: MAE has 3.46x the weight of RMSE.
GAP TO RANK 1: only 0.00011 MAE improvement needed.

STRATEGY:
  - XGBoost with MAE loss (objective='reg:absoluteerror')
    => Algo diversity + directly optimizes the dominant metric term
  - CatBoost-MAE-1 (d5, lr=0.03, s42) -- v17's best individual model
  - CatBoost-MAE-2 (d5, lr=0.05, s789) -- seed diversity
  - CatBoost-RMSE (d5, lr=0.03, s123) -- loss diversity anchor
  - Ridge stacking (proven for generalization)

WHY THIS SHOULD WIN:
  v13 (best LB): XGB(RMSE) + LGB(RMSE) + CAT(RMSE) => Ridge
  v17 (best local): 3x CAT(MAE) => simple average
  v19: XGB(MAE!) + 2x CAT(MAE) + CAT(RMSE) => Ridge
       = v13's algorithm diversity + v17's MAE insight
"""
...
# -----------------------------------------------------------------
print("=" * 70)
print("  ML OPSIDIAN v19 - METRIC-INFORMED GOLDILOCKS")
print("=" * 70)
print("\n[LOAD] Loading data...")
...
print("\n" + "=" * 70)
print("  5-FOLD CV -- XGB(MAE) + 2x CAT(MAE) + CAT(RMSE) -> Ridge")
print("  Metric-informed: MAE has 3.46x the weight of RMSE")
print("=" * 70)
...
for fold, (tr_idx, va_idx) in enumerate(skf.split(train_df, y_bins)):
    ...
    # === 2. CatBoost-MAE-1 (d5, lr=0.03, seed=42) — v17's best ===
    ...
    print(f"   [CAT-MAE-1]   iter={cat_mae1.best_iteration_}  (d5, lr=0.03, s42)")

    # === 3. CatBoost-MAE-2 (d5, lr=0.05, seed=789) — seed diversity ===
    ...
    print(f"   [CAT-MAE-2]   iter={cat_mae2.best_iteration_}  (d5, lr=0.05, s789)")

    # === 4. CatBoost-RMSE (d5, lr=0.03, seed=123) — loss diversity anchor ===
    ...
    print(f"   [CAT-RMSE]    iter={cat_rmse.best_iteration_}  (d5, lr=0.03, s123)")
...
for name in MODEL_NAMES:
    ...
    # Predicted LB using reverse-engineered formula
...
# MAE-only average (3 MAE models)
mae_names = [m for m in MODEL_NAMES if "MAE" in m]
oof_mae = np.mean([oof_preds[m] for m in mae_names], axis=0)
...
print("\n" + "=" * 70)
print("  GLOBAL OOF RESULTS (v19 - Metric-Informed Goldilocks)")
print("=" * 70)
...
# fold_report.to_csv("submissions/fold_report_v19.csv", index=False)
print(f"\n[DONE] Saved fold_report_v19.csv")
print(fold_report.to_string(index=False))

# Comparison with estimated LBs
print(f"\n[COMPARE]")
print(f"  v13          : MAE=0.17937, RMSE=0.23500, EV=0.03060  LB=0.38476 (actual)")
print(f"  v17          : MAE=0.17882, RMSE=0.23465, EV=0.03390  LB=0.38506 (actual)")
print(f"  v19 Current  : MAE={g_mae:.5f}, RMSE={g_rmse:.5f}, EV={g_ev:.5f}  est_LB={g_lb:.5f}")

# Gap analysis
print(f"\n[GAP TO RANK 1]")
print(f"  Rank 1 LB:     0.38215")
print(f"  v19 est. LB:   {g_lb:.5f}")
print(f"  Est. gap:      {g_lb - 0.38215:+.5f}")
...
# submission.to_csv("submissions/submission_v19.csv", index=False)
print(f"\n[DONE] Saved submission_v19.csv ({len(submission)} rows)")
print(f"       Pred range : [{tst_final.min():.4f}, {tst_final.max():.4f}]")

# Save OOF predictions for standalone evaluation
# np.save("submissions/oof_v19.npy", best_oof)
print(f"[DONE] Saved oof_v19.npy (for evaluate.py)")


sub_v19 = submission.copy()
del train_df, test_df, fold_report, submission  # Clear memory
print('=' * 70)
print('  STARTING PHASE 2: ML OPSIDIAN v20')
print('=' * 70)

"""
ML Opsidian: Genesis v20 - Advanced MLOps Techniques
======================================================
Enhancements:
1. Precision Fingerprint (lat_decimal_len, lon_decimal_len)
2. Target Quantile Encoding (25th and 75th percentiles)
3. Semi-Supervised Pseudo-Labeling (from v19 predictions)
4. Spatial GroupKFold (by grid_id) to prevent spatial leakage
"""
...
# -----------------------------------------------------------------
print("=" * 70)
print("  ML OPSIDIAN v20 - ADVANCED MLOPS")
print("=" * 70)
print("\n[LOAD] Loading data...")
train_df = pd.read_csv(os.path.join(DATA_DIR, "train.csv"))
test_df  = pd.read_csv(os.path.join(DATA_DIR, "test.csv"))
...
print("\n[SEMI-SUPERVISED] Pseudo-Labeling from v19...")
# sub_v19 is already in memory from Phase 1
test_pseudo = test_df.merge(sub_v19, on='record_id', how='left')
...
# Note: We must NOT evaluate on pseudo-labeled rows. They are for training only.
oof_preds = {m: np.zeros(len(train_df)) for m in MODEL_NAMES}
tst_preds = {m: np.zeros(len(test_df))  for m in MODEL_NAMES}
fold_results = []

print("\n" + "=" * 70)
print("  5-FOLD SPATIAL GROUP CV -- XGB(MAE) + 2x CAT(MAE) + CAT(RMSE) -> Ridge")
print("=" * 70)
...
for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
    ...
    # Remove pseudo labels from validation set if they leaked in
    ...
    # Target encodings (only learn from REAL training data, not pseudo)
...
# Ridge stacking with CV (Using real mask)
oof_stacked = np.zeros(len(train_df))
tst_stacked_accum = np.zeros(len(test_df))
for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
    va_is_pseudo = train_df.iloc[va_idx]['is_pseudo'] == 1
...
ridge_final = Ridge(alpha=1.0, fit_intercept=True)
ridge_final.fit(oof_meta, y_arr) # fit on all real data
tst_stacked = ridge_final.predict(tst_meta)
...
print("\n" + "=" * 70)
print("  GLOBAL OOF RESULTS (v20 - Advanced MLOps)")
print("=" * 70)
print(f"    MAE            : {g_mae:.5f}")
print(f"    RMSE           : {g_rmse:.5f}")
print(f"    Explained Var. : {g_ev:.5f}")
...
fold_report = pd.DataFrame(fold_results)
# fold_report.to_csv("submissions/fold_report_v20.csv", index=False)
print(f"\n[DONE] Saved fold_report_v20.csv")
...
# submission.to_csv("submissions/submission_v20.csv", index=False)
print(f"\n[DONE] Saved submission_v20.csv ({len(submission)} rows)")

# np.save("submissions/oof_v20.npy", best_oof)
print(f"[DONE] Saved oof_v20.npy (for evaluate.py)")
...
"""
ML Opsidian: Genesis v24 - Median Align, Multi-Seed & Interaction Synergy
========================================================================
Enhancements over v20:
1. Median-Based Target Encoding (Alignment with MAE)
2. Target Variance Feature (Uncertainty Calibration)
3. Composite Interaction Features (Granular Geography)
4. CatBoost max_ctr_complexity=2 (Internal Interactions)
5. Multi-Seed Averaging (Variance Reduction, seeds: 42, 123, 456, 789)
6. Update pseudo-label source from v19 -> v20 (window 0.45-0.50)
7. Positive constraint on Ridge Level-2 stacker
"""
...
# -----------------------------------------------------------------
print("=" * 70)
print("  ML OPSIDIAN v24 - TARGETED OVERFIT ROLLBACK")
print("=" * 70)
print("\n[LOAD] Loading data...")
train_df = pd.read_csv(os.path.join(DATA_DIR, "train.csv"))
test_df  = pd.read_csv(os.path.join(DATA_DIR, "test.csv"))
...
# sub_v20 is already in memory from Phase 2
test_pseudo = test_df.merge(sub_v20, on='record_id', how='left')

# Filter highly confident predictions
mask = (test_pseudo['flood_risk_score'] <= 0.35) | (test_pseudo['flood_risk_score'] >= 0.60)
pseudo_rows = test_pseudo[mask].copy()
pseudo_rows['is_pseudo'] = 1
train_df['is_pseudo'] = 0
test_df['is_pseudo'] = 0
...
IGNORE_COLS = DROP_COLS + [TARGET, "flood_occurrence_yes", "lat_decimal_len", "lon_decimal_len",
                           "downstream_sig", "infra_deficit_sig"]
SPATIAL_HELPERS = ["lat_bin", "lon_bin", "grid_id"]
BASE_FEATURES = [c for c in train_df.columns if c not in IGNORE_COLS and c not in SPATIAL_HELPERS]
...
for seed_idx, seed in enumerate(SEEDS):
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        # Combine all columns that need median encoding
    ...
    # 7. Positive constraint on Ridge Level-2 stacker
    ridge_seed = Ridge(alpha=1.0, fit_intercept=True, positive=True)
    ...
    oof_predictions_accum[real_mask] += oof_stacked_seed / len(SEEDS)
...
print("\n" + "=" * 70)
print("  GLOBAL MULTI-SEED RESULTS (v24 - Targeted 7-Point Upgrade)")
print("=" * 70)
print(f"    MAE            : {g_mae:.5f}")
print(f"    RMSE           : {g_rmse:.5f}")
print(f"    Explained Var. : {g_ev:.5f}")
...
fold_report.to_csv("fold_report_v24.csv", index=False)
print(f"\n[DONE] Saved fold_report_v24.csv")
...
print(f"[DONE] Saved submission_v24.csv ({len(submission)} rows)")

np.save("oof_v24.npy", oof_predictions_accum[real_mask])
print(f"[DONE] Saved oof_v24.npy (for evaluate.py)")
