"""
ML Opsidian: Genesis v29 - LightGBM Integration, Unweighted Stacking & Corrected Metric scoring
========================================================================
Features of v29:
...
5. Unweighted training (sample weight = 1.0) to prevent overfitting to geospatial imputation noise in real rows.
6. Addition of LGBM-MAE (LightGBM with MAE objective) to add diversity to the Level-1 stack.
7. Corrected metric scoring using the true multiplicative leaderboard formula.
...
# -----------------------------------------------------------------
print("=" * 70)
print("  ML OPSIDIAN v29 - LGBM INTEGRATION & UNWEIGHTED CALIBRATION")
print("=" * 70)
print("\n[LOAD] Loading data...")
...
# -----------------------------------------------------------------
# 2. FEATURE ENGINEERING (Reverted to v24 base)
# -----------------------------------------------------------------
...
print("\n" + "=" * 70)
print(f"  5-FOLD STRATIFIED TRAINING (SINGLE SEED 42) - UNWEIGHTED v29")
print("=" * 70)
...
for fold, (tr_idx, va_idx) in enumerate(skf.split(train_df, y_bins)):
    ...
    oof_preds["LGB-MAE (d5)"][va_idx] = lgb_mae.predict(X_va)
...
# -----------------------------------------------------------------
# 5. RIDGE/LINEAR STACKING (Leak-Free Nested CV)
# -----------------------------------------------------------------
print("\n" + "=" * 70)
...
for fold, (tr_idx, va_idx) in enumerate(skf.split(train_df, y_bins)):
    ...
    stacker.fit(oof_meta[tr_idx], y_arr[tr_idx])
...
final_stacker.fit(oof_meta, y_arr)
...
# -----------------------------------------------------------------
# 6. GLOBAL ENSEMBLE RESULTS & METRICS
# -----------------------------------------------------------------
...
print("\n" + "=" * 70)
print("  GLOBAL OOF RESULTS (v29 - LGBM MAE & Unweighted Stacking)")
print("=" * 70)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v29.csv", index=False)
fold_report.to_csv("submissions/fold_report_v29.csv", index=False)
print(f"\n[DONE] Saved fold_report_v29.csv and submissions/fold_report_v29.csv")
...
submission.to_csv("submission_v29.csv", index=False)
submission.to_csv("submissions/submission_v29.csv", index=False)
print(f"[DONE] Saved submission_v29.csv and submissions/submission_v29.csv ({len(submission)} rows)")

np.save("oof_v29.npy", oof_stacked)
np.save("submissions/oof_v29.npy", oof_stacked)
print(f"[DONE] Saved oof_v29.npy and submissions/oof_v29.npy (for evaluate.py)")
