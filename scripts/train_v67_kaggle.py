"""
ML Opsidian: Genesis v67 - Multi-Seed & Conflict-Resolved Stacking Pipeline
==================================================================================
Features of v67:
Inherits v42 ( Breakthrough Spatial Stacking & Pseudo-Labeling Pipeline) with two modifications:
1. Multi-Seed Ensembling: SEEDS = [42, 2024]
2. Leak-Free Target Conflict Resolution: applied to tr_rows[TARGET] inside the fold loop.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v67 - MULTI-SEED & CONFLICT-RESOLVED PIPELINE")
print("=" * 75)
...
# [v67] Pre-processing Conflict Resolution Column Definitions (No global overwrite of train_df[TARGET])
downstream_cols = [
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v67 PIPELINE")
print("=" * 75)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        # [v67] Apply conflict resolution ONLY on the training fold (tr_rows)
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v67 - Raw Custom Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v67.csv", index=False)
fold_report.to_csv("submissions/fold_report_v67.csv", index=False)
...
submission.to_csv("submission_v67.csv", index=False)
submission.to_csv("submissions/submission_v67.csv", index=False)
print(f"[DONE] Saved submission_v67.csv ({len(submission)} rows)")

np.save("oof_v67.npy", all_oof_stacked)
np.save("submissions/oof_v67.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v67.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v67)")
print("=" * 75)
...
np.save("oof_v67_optimized.npy", opt_oof)
np.save("submissions/oof_v67_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v67_optimized.npy")
...
submission_opt.to_csv("submission_v67_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v67_optimized.csv", index=False)
print(f"[DONE] Saved submission_v67_optimized.csv ({len(submission_opt)} rows)")
