"""
ML Opsidian: Genesis v705 - isolates corrected coefficients on top of v703 peak baseline
==================================================================================
Base: v703 (v70 + Value Frequency Count Features, LB 0.382030 — Project Peak)

Changes vs v703 (1 targeted improvement):
...
Other changes from v704 (extended FREQ_COLS and upgraded pseudo-labels)
are rolled back to isolate the coefficient update effect on the champion v703 base.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v705 - ISOLATED CORRECTED COEFFICIENTS")
print("=" * 75)
...
fold_report.to_csv("fold_report_v705.csv", index=False)
fold_report.to_csv("submissions/fold_report_v705.csv", index=False)
...
submission.to_csv("submission_v705.csv", index=False)
submission.to_csv("submissions/submission_v705.csv", index=False)
print(f"[DONE] Saved submission_v705.csv ({len(submission)} rows)")

np.save("oof_v705.npy", all_oof_stacked)
np.save("submissions/oof_v705.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v705.npy")

# -----------------------------------------------------------------
# 8. POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v705)")
print("=" * 75)
...
np.save("oof_v705_optimized.npy", opt_oof)
np.save("submissions/oof_v705_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v705_optimized.npy")
...
submission_opt.to_csv("submission_v705_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v705_optimized.csv", index=False)
print(f"[DONE] Saved submission_v705_optimized.csv ({len(submission_opt)} rows)")
