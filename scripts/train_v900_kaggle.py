"""
ML Opsidian: Genesis v900 - Oracle & Nodes Architecture
==================================================================================
Base: v703
New features:
1. Downstream Oracle (7th Model) - CatBoost strictly blinded to physical features.
2. HistGradientBoostingRegressor (8th Model) - True algorithmic diversity.
3. Joint Lat/Lon frequency counts to fingerprint synthetic grid nodes.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v900 - ORACLE & NODES ARCHITECTURE")
print("=" * 75)
...
combined_raw['lat_lon'] = combined_raw['latitude'].astype(str) + '_' + combined_raw['longitude'].astype(str)
joint_map = combined_raw['lat_lon'].value_counts().to_dict()
for df in [train_df, test_df]:
    df['lat_lon_joint_freq'] = (df['latitude'].astype(str) + '_' + df['longitude'].astype(str)).map(joint_map).fillna(0).astype(float)
print(f"   lat_lon_joint_freq: {len(joint_map)} unique grid nodes")
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v900 PIPELINE")
print("=" * 75)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        oracle_cols = [c for c in FEATURES if any(k in c for k in downstream_base)]
        X_tr_oracle = X_tr_cat[oracle_cols].copy()
        X_va_oracle = X_va_cat[oracle_cols].copy()
        X_te_oracle = X_te_cat[oracle_cols].copy()
        cat_cols_oracle = [c for c in cat_cols if c in oracle_cols]
        ...
        cat_oracle = cb.CatBoostRegressor(
            iterations=3000, learning_rate=0.03, depth=3, l2_leaf_reg=10.0,
            loss_function="MAE", eval_metric="MAE", task_type="CPU",
            random_seed=seed + 4, verbose=False
        )
        cat_oracle.fit(X_tr_oracle, y_tr, cat_features=cat_cols_oracle, eval_set=(X_va_oracle, y_va), early_stopping_rounds=150, verbose=False)

        # 8. HGBR-MAE
        hgbr = HistGradientBoostingRegressor(
            loss='absolute_error', learning_rate=0.03, max_iter=2000,
            max_depth=5, min_samples_leaf=40, l2_regularization=5.0,
            categorical_features=cat_cols, random_state=seed + 5,
        ...
        oof_preds["CAT-ORACLE (d3)"][va_idx_clean] = cat_oracle.predict(X_va_oracle)
        oof_preds["HGBR-MAE"][va_idx_clean]       = hgbr.predict(X_va)
        ...
        tst_preds["CAT-ORACLE (d3)"] += cat_oracle.predict(X_te_oracle) / N_FOLDS
        tst_preds["HGBR-MAE"]       += hgbr.predict(X_te)        / N_FOLDS
        ...
        print(f"      Fold {fold+1}/5 | XGB1={xgb_m1.best_iteration:<4} CAT1={cat_m1.best_iteration_:<4} CAT2={cat_m2.best_iteration_:<4} CAT3={cat_rmse.best_iteration_:<4} LGB={lgb_m1.best_iteration_:<4} XGB2={xgb_m2.best_iteration:<4} ORC={cat_oracle.best_iteration_:<4} | [ENS MAE={f_mae:.4f}] [{time.time() - t0:.0f}s]")
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v900 - Oracle & Nodes)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v900.csv", index=False)
fold_report.to_csv("submissions/fold_report_v900.csv", index=False)
...
submission.to_csv("submission_v900.csv", index=False)
submission.to_csv("submissions/submission_v900.csv", index=False)
print(f"[DONE] Saved submission_v900.csv ({len(submission)} rows)")

np.save("oof_v900.npy", all_oof_stacked)
np.save("submissions/oof_v900.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v900.npy")

# -----------------------------------------------------------------
# 8. POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v900)")
print("=" * 75)
...
np.save("oof_v900_optimized.npy", opt_oof)
np.save("submissions/oof_v900_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v900_optimized.npy")
...
submission_opt.to_csv("submission_v900_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v900_optimized.csv", index=False)
print(f"[DONE] Saved submission_v900_optimized.csv ({len(submission_opt)} rows)")
