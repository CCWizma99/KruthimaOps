"""
ML Opsidian: Genesis v58 - Leak-Free Conflict Resolution & Honest Validation
==============================================================================
Upgrades from v55:
1. Shifted target conflict resolution inside the K-Fold CV loop to apply it only to tr_rows[TARGET].
2. Keeps train_df[TARGET] raw/un-smoothed globally during CV loop to prevent target leakage.
3. Evaluates OOF predictions, stacking, L2 nested search, and post-hoc optimization/blending strictly on raw targets.
"""
...
USE_BLEND = True  # Enabled for v58
...
print("=" * 75)
print("  ML OPSIDIAN v58 - LEAK-FREE CONFLICT RESOLUTION & HONEST VALIDATION")
print("=" * 75)
...
# -----------------------------------------------------------------
# 1.6. SEMI-SUPERVISED PSEUDO-LABELING (v58 priority)
# -----------------------------------------------------------------
...
COMPOSITE_ENC_COLS = [] # Removed downstream_district_sig
STD_ENC_COLS = [
...
# [v58] Pre-processing Conflict Resolution Column Definitions (No global overwrite of train_df[TARGET])
downstream_cols = [
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - SINGLE-SEED v58 PIPELINE")
print("=" * 75)
...
for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
    ...
    # [v58] Apply conflict resolution ONLY on the training fold (tr_rows)
...
# Construct Stacking Meta-Feature Matrices (Analytical Median removed to avoid multicollinearity)
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v58 - Raw Stacking)")
print("=" * 75)
...
fold_report.to_csv("fold_report_v58.csv", index=False)
fold_report.to_csv("submissions/fold_report_v58.csv", index=False)
...
submission.to_csv("submissions/submission_v58.csv", index=False)
print(f"[DONE] Saved submission_v58.csv ({len(submission)} rows)")

np.save("oof_v58.npy", oof_stacked)
np.save("submissions/oof_v58.npy", oof_stacked)
print(f"[DONE] Saved oof_v58.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED FALLBACK-ENABLED PER-GROUP POWER TRANSFORMATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v58)")
print("=" * 75)
...
if USE_BLEND:
    print("\n" + "=" * 75)
    print("  DYNAMIC ANALYTICAL GROUP MEDIAN BLEND (v58)")
    print("=" * 75)
...
np.save("oof_v58_optimized.npy", final_oof)
np.save("submissions/oof_v58_optimized.npy", final_oof)
print(f"[DONE] Saved oof_v58_optimized.npy")
...
submission_opt.to_csv("submission_v58_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v58_optimized.csv", index=False)
print(f"[DONE] Saved submission_v58_optimized.csv ({len(submission_opt)} rows)")
