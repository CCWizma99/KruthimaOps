"""
ML Opsidian: Genesis vmal - Robust Customized Imputation + Adversarial Drop + 5 Seeds
==================================================================================
Base: v703 (v70 + Value Frequency Count Features, LB 0.382030 — Project Peak)

Major Enhancements:
1. ADVANCED CUSTOMIZED IMPUTATION FRAMEWORK:
   - Removes the artificial 'missing' category for nominal variables by imputing
     missing categories with the District Mode.
   - Replaces coarse global median imputation for environmental and demographic 
     variables with localized District/Landcover/Soil medians and KNN coordinate regressions.
2. ADVERSARIAL FEATURE DROP:
   - Drops 'extreme_weather_index' to resolve covariate shift (AV AUC = 0.6378).
   - Removes associated features: 'cyclone_vulnerability' and 'flood_pressure'.
3. UPGRADED PSEUDO-LABELS:
   - Changes pseudo-label source to submission_v703.csv (raw, unoptimized)
     to prevent scale/calibration mismatch.
4. prediction STABILITY:
   - Set 5 seeds (SEEDS = [42, 2024, 7, 100, 999]).
   - Stacker optimized via stable coefficients (c_mae=0.539328, c_rmse=1.152263, c_ev=0.048467)
     to prevent L2 stacker overfitting on validation fold EV noise, while using true refitted
     coefficients (c_mae=0.544177, c_rmse=1.148953, c_ev=0.049625, c_int=-0.000580) for evaluation.
"""
...
print("=" * 75)
print("  ML OPSIDIAN vmal - CUSTOMIZED IMPUTATION + ADVERSARIAL DROP")
print("=" * 75)
...
# [CHANGE] Added extreme_weather_index to IGNORE_COLS to drop it from features
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED vmal PIPELINE")
print("=" * 75)
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (vmal - Customized Imputation)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_vmal.csv", index=False)
fold_report.to_csv("submissions/fold_report_vmal.csv", index=False)
...
submission.to_csv("submission_vmal.csv", index=False)
submission.to_csv("submissions/submission_vmal.csv", index=False)
print(f"[DONE] Saved submission_vmal.csv ({len(submission)} rows)")

np.save("oof_vmal.npy", all_oof_stacked)
np.save("submissions/oof_vmal.npy", all_oof_stacked)
print(f"[DONE] Saved oof_vmal.npy")

# -----------------------------------------------------------------
# 8. POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (vmal)")
print("=" * 75)
...
np.save("oof_vmal_optimized.npy", opt_oof)
np.save("submissions/oof_vmal_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_vmal_optimized.npy")
...
submission_opt.to_csv("submission_vmal_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_vmal_optimized.csv", index=False)
print(f"[DONE] Saved submission_vmal_optimized.csv ({len(submission_opt)} rows)")
