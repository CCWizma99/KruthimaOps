"""
ML Opsidian: Genesis v64 - Target Power Transformation Stacking Pipeline
==================================================================================
Features of v64:
Inherits v42 ( Breakthrough Spatial Stacking & Pseudo-Labeling Pipeline) with a single change:
- Global Yeo-Johnson Power Transformation on the target variable before model training.
- Inverse-transforms predictions back to original space for evaluation and stacking.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v64 - TARGET POWER TRANSFORM PIPELINE")
print("=" * 75)
...
# -----------------------------------------------------------------
# [v64] Keep a copy of original target values for validation evaluation
train_df['original_flood_risk_score'] = train_df[TARGET].copy()

# [v64] Fit and apply Yeo-Johnson PowerTransformer globally
...
# [v64] original_y and original_df targets remain untransformed/raw
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v64 PIPELINE")
print("=" * 75)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        # [v64] Predictions - generate in transformed space, and inverse-transform back to original space
        ...
        # [v64] Evaluate against original un-transformed validation target
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v64 - Raw Custom Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v64.csv", index=False)
fold_report.to_csv("submissions/fold_report_v64.csv", index=False)
...
submission.to_csv("submission_v64.csv", index=False)
submission.to_csv("submissions/submission_v64.csv", index=False)
print(f"[DONE] Saved submission_v64.csv ({len(submission)} rows)")

np.save("oof_v64.npy", all_oof_stacked)
np.save("submissions/oof_v64.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v64.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v64)")
print("=" * 75)
...
np.save("oof_v64_optimized.npy", opt_oof)
np.save("submissions/oof_v64_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v64_optimized.npy")
...
submission_opt.to_csv("submission_v64_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v64_optimized.csv", index=False)
print(f"[DONE] Saved submission_v64_optimized.csv ({len(submission_opt)} rows)")
