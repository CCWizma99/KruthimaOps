"""
ML Opsidian: Genesis v707 - stable stacker, 5 seeds, and Kaggle tabular hacks
==================================================================================
Base: v706 (v703 + stable stacker, 5 seeds, inundation_ratio_cat, target profiling)

Changes vs v706 (2 improvements):
1. Continuous-to-categorical alignment for topographic wetness index: rounded pseudo_twi to 3 decimal places and cast as str category (pseudo_twi_cat).
2. Value counts "magic feature" for inundation_ratio (inundation_ratio_freq) to uncover synthetic generation grid snaps.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v707 - STABLE STACKER + 5 SEEDS + TABULAR HACKS")
print("=" * 75)
...
def engineer_features(df):
    ...
    df['inundation_ratio_cat'] = df['inundation_ratio'].round(3).astype(str)
    df['pseudo_twi_cat'] = df['pseudo_twi'].round(3).astype(str)
...
# Compute value frequency counts for inundation_ratio
print("\n[FEAT] Computing value frequency count for inundation_ratio...")
combined_inundation = pd.concat([train_df['inundation_ratio'], test_df['inundation_ratio']], ignore_index=True)
inundation_freq_map = combined_inundation.value_counts().to_dict()
for df in [train_df, test_df]:
    df['inundation_ratio_freq'] = df['inundation_ratio'].map(inundation_freq_map).fillna(0).astype(float)
...
CAT_FEATURES = [
    ...
    "inundation_ratio_cat", "pseudo_twi_cat"
]
...
freq_feature_names = [f'{col}_freq' for col in FREQ_COLS if f'{col}_freq' in train_df.columns] + ['inundation_ratio_freq']
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v707 PIPELINE")
print("=" * 75)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        # Target distribution profiling (ai-says.txt Deotte Distribution Profile)
        ...
        # Count frequency distribution per target bin for each downstream_sig group
        hist_counts = tr_rows.groupby(['downstream_sig', 'target_bin']).size().unstack(fill_value=0)
        ...
        hist_features = []
        for b in range(5):
            feature_name = f'downstream_sig_hist_bucket_{b}'
            hist_features.append(feature_name)
            for df_split in [tr_rows, va_rows, test_df]:
                df_split[feature_name] = df_split['downstream_sig'].map(
...
fold_report.to_csv("fold_report_v707.csv", index=False)
fold_report.to_csv("submissions/fold_report_v707.csv", index=False)
...
submission.to_csv("submission_v707.csv", index=False)
submission.to_csv("submissions/submission_v707.csv", index=False)
print(f"[DONE] Saved submission_v707.csv ({len(submission)} rows)")

np.save("oof_v707.npy", all_oof_stacked)
np.save("submissions/oof_v707.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v707.npy")

# -----------------------------------------------------------------
# 8. POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v707)")
print("=" * 75)
...
np.save("oof_v707_optimized.npy", opt_oof)
np.save("submissions/oof_v707_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v707_optimized.npy")
...
submission_opt.to_csv("submission_v707_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v707_optimized.csv", index=False)
print(f"[DONE] Saved submission_v707_optimized.csv ({len(submission_opt)} rows)")
