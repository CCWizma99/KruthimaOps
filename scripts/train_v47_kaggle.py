"""
ML Opsidian: Genesis v47 - Topography & Monsoon Feature Maximization Pipeline
==========================================================================
Upgrades from v46:
1. Maximized Topographic Features:
   - Nuwara Eliya highland runoff potential.
   - Coastal drainage risk proxy.
   - District-relative elevation basins ('is_district_lowpoint').
2. Meteorological Exposure:
   - Windward SW Yala monsoon precipitation risk.
   - Windward NE Maha monsoon precipitation risk.
3. Pseudo-Label Source:
   - Uses `submission_v46.csv` (raw un-calibrated scale from v46) as soft pseudo-labels.
4. Pipeline Mechanics:
   - Retains LOO encoding, 5-quantile CatBoost ensemble, group calibration, and analytical blend.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v47 - GEOGRAPHY & MONSOON MAXIMIZATION PIPELINE")
print("=" * 75)
...
if USE_PSEUDO:
    # Use RAW un-calibrated v46 predictions to prevent double-calibration of test coordinates
    pseudo_path = "submission_v46.csv"
    if not os.path.exists(pseudo_path):
        pseudo_path = "submissions/submission_v46.csv"
    ...
    if os.path.exists(pseudo_path):
        ...
        print("   [WARNING] submission_v46.csv not found. Skipping pseudo-labeling. Will fallback to v45 raw.")
        pseudo_path_fallback = "submissions/submission_v45.csv"
        if os.path.exists(pseudo_path_fallback):
            sub_blend = pd.read_csv(pseudo_path_fallback)
            test_pseudo = test_df.merge(sub_blend, on="record_id", how="left")
            pseudo_rows = test_pseudo.copy()
            pseudo_rows['is_pseudo'] = 1
            print(f"   Added {len(pseudo_rows)} soft pseudo-labeled rows from test set (Fallback v45).")
            train_df = pd.concat([train_df, pseudo_rows], ignore_index=True)
...
# Recalculate combined mapping references after imputation
combined = pd.concat([
    train_df.drop(columns=['flood_risk_score'], errors='ignore'), test_df
], ignore_index=True)
...
district_med_elev = combined.groupby('district')['elevation_m'].median().to_dict()
...
def engineer_features(df):
    ...
    # Seasonal Windward / Leeward Monsoon Exposures
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v47 PIPELINE")
print("=" * 75)

t_start_global = time.time()

# -----------------------------------------------------------------
# 5.2. LEAVE-ONE-OUT TARGET ENCODING HELPER
# -----------------------------------------------------------------
def loo_target_encode(tr_df, va_df, te_df, col, target, global_median, smoothing=10):
    ...
    # Cast to string/object to avoid pandas 2.0+ Categorical Index mapping bugs
    tr_col_str = tr_df[col].astype(str)
    va_col_str = va_df[col].astype(str)
    te_col_str = te_df[col].astype(str)
    
    stats = real_tr.groupby(real_tr[col].astype(str))[target].agg(['sum', 'count'])
    
    # Map stats to train rows and cast to float
    tr_sum = tr_col_str.map(stats['sum']).fillna(0).astype(float)
    tr_count = tr_col_str.map(stats['count']).fillna(0).astype(float)
    ...
    va_df[f'{col}_loo_enc'] = va_col_str.map(full_enc).fillna(global_median).astype(float)
    te_df[f'{col}_loo_enc'] = te_col_str.map(full_enc).fillna(global_median).astype(float)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        loo_features = ['downstream_sig_loo_enc', 'district_loo_enc']

        # Combine all features
        FEATURES = BASE_FEATURES + te_features + loo_features
        ...
        xgb_m1 = xgb.XGBRegressor(
            n_estimators=2500, learning_rate=0.05, max_depth=6, min_child_weight=4,
        ...
        cat_m1 = cb.CatBoostRegressor(
            iterations=3000, learning_rate=0.05, depth=5, l2_leaf_reg=5.0,
            bagging_temperature=0.7, random_strength=2.0, border_count=254,
            loss_function="MAE", eval_metric="MAE", task_type="CPU", thread_count=-1,
        ...
        cat_m2 = cb.CatBoostRegressor(
            iterations=3000, learning_rate=0.05, depth=5, l2_leaf_reg=12.0,
        ...
        cat_rmse = cb.CatBoostRegressor(
            iterations=2500, learning_rate=0.05, depth=5, l2_leaf_reg=8.0,
            bagging_temperature=0.6, random_strength=3.0, border_count=254,
            loss_function="RMSE", eval_metric="RMSE", task_type="CPU", thread_count=-1,
        ...
        lgb_m1 = lgb.LGBMRegressor(
            n_estimators=3000, learning_rate=0.05, num_leaves=31, max_depth=5,
        ...
        xgb_m2 = xgb.XGBRegressor(
            n_estimators=2500, learning_rate=0.05, max_depth=5, min_child_weight=6,
        ...
        # 7. Quantile Ensemble Base Model (CPU-Optimized with LightGBM)
        quantile_preds_va = []
        quantile_preds_te = []
        for alpha in QUANTILE_ALPHAS:
            qmodel = lgb.LGBMRegressor(
                n_estimators=2500, learning_rate=0.05, max_depth=5, num_leaves=31,
                objective="quantile", alpha=alpha, random_state=seed, n_jobs=-1, verbosity=-1
            )
            qmodel.fit(X_tr, y_tr, eval_set=[(X_va, y_va)], callbacks=[lgb.early_stopping(100, verbose=False)])
            quantile_preds_va.append(qmodel.predict(X_va))
            quantile_preds_te.append(qmodel.predict(X_te))
            
        va_quantile_ensemble = np.median(quantile_preds_va, axis=0)
        te_quantile_ensemble = np.median(quantile_preds_te, axis=0)

        # --- Save Out-Of-Fold Predictions ---
        oof_preds["XGB-MAE-1 (d6)"][va_idx_clean] = xgb_m1.predict(X_va_xgb)
        ...
        oof_preds["LGB-Quantile-Median"][va_idx_clean] = va_quantile_ensemble

        # --- Accumulate Test Predictions ---
        tst_preds["XGB-MAE-1 (d6)"] += xgb_m1.predict(X_te_xgb) / N_FOLDS
        ...
        tst_preds["LGB-Quantile-Median"] += te_quantile_ensemble / N_FOLDS

        # --- Compute CV-Safe Analytical Group Medians ---
        sig_stats = tr_rows.groupby('downstream_sig')[TARGET].agg(['median', 'count'])
    ...
    print(f"   [L2 SEARCH SEED {seed}] Best alpha: {best_alpha_l2} (Nested CV Metric Score: {best_score_l2:.5f})")
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf_l2.split(original_df, original_y, original_groups)):
        w_fold, b_fold = fit_metric_stacker(oof_meta_seed[tr_idx], original_y[tr_idx], alpha=best_alpha_l2)
    ...
    w_final_seed, b_final_seed = fit_metric_stacker(oof_meta_seed, original_y, alpha=best_alpha_l2)
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v47 - Raw Stacking)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v47.csv", index=False)
fold_report.to_csv("submissions/fold_report_v47.csv", index=False)
...
submission.to_csv("submission_v47.csv", index=False)
submission.to_csv("submissions/submission_v47.csv", index=False)
print(f"[DONE] Saved submission_v47.csv ({len(submission)} rows)")

np.save("oof_v47.npy", all_oof_stacked)
np.save("submissions/oof_v47.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v47.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED FALLBACK-ENABLED PER-GROUP POWER TRANSFORMATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v47)")
print("=" * 75)
...
print(f"[DONE] Saved submission_v47_optimized.csv ({len(submission_opt)} rows)")
