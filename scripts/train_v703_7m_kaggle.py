"""
ML Opsidian: Genesis v703_7m - v703 with both CAT-RMSE and CAT-HUBER (7 Models)
=============================================================================
Base: v703 (6-Model, 3-Seed, Value Frequency Counts, Target Denoising, Dirty Imputation)
Exactly 1 change vs v703:
  - Adds CAT-HUBER (d6) as a 7th model while retaining CAT-RMSE (d5).
"""
...
print("=" * 75)
print("  ML OPSIDIAN v703_7m - v703 with RMSE + HUBER (7 Models, 3 SEEDS)")
print("=" * 75)
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - MULTI-SEED v703_7m PIPELINE")
print("=" * 75)
...
for seed in SEEDS:
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
        ...
        print(f"      Fold {fold+1}/5 | XGB1={xgb_m1.best_iteration:<4} CAT1={cat_m1.best_iteration_:<4} CAT2={cat_m2.best_iteration_:<4} RMSE={cat_rmse.best_iteration_:<4} HUB={cat_hub.best_iteration_:<4} LGB={lgb_m1.best_iteration_:<4} XGB2={xgb_m2.best_iteration:<4} | [ENS MAE={f_mae:.4f}] [{time.time() - t0:.0f}s]")
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v703_7m - v703 with RMSE + HUBER)")
print("=" * 75)
print(f"    [ALL ROWS]")
...
fold_report.to_csv("fold_report_v703_7m.csv", index=False)
fold_report.to_csv("submissions/fold_report_v703_7m.csv", index=False)
...
submission.to_csv("submission_v703_7m.csv", index=False)
submission.to_csv("submissions/submission_v703_7m.csv", index=False)
print(f"[DONE] Saved submission_v703_7m.csv ({len(submission)} rows)")

np.save("oof_v703_7m.npy", all_oof_stacked)
np.save("submissions/oof_v703_7m.npy", all_oof_stacked)
print(f"[DONE] Saved oof_v703_7m.npy")

# -----------------------------------------------------------------
# 8. POST-HOC POWER TRANSFORMATION OPTIMIZATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v703_7m)")
print("=" * 75)
...
np.save("oof_v703_7m_optimized.npy", opt_oof)
np.save("submissions/oof_v703_7m_optimized.npy", opt_oof)
print(f"[DONE] Saved oof_v703_7m_optimized.npy")
...
submission_opt.to_csv("submission_v703_7m_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v703_7m_optimized.csv", index=False)
print(f"[DONE] Saved submission_v703_7m_optimized.csv ({len(submission_opt)} rows)")
