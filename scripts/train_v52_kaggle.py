"""
ML Opsidian: Genesis v52 - Compound Interaction + Analytical Fusion Pipeline
==============================================================================
Upgrades from v51:
1. Compound `downstream_district_sig` feature (TE + LOO).
2. Inundation Quantile Bucket feature `inundation_sig`.
3. `max_ctr_complexity=2` added to all CatBoost models.
4. Analytical Median as unconstrained 11th Meta-Feature in Level-2 Stacker.
5. Enabled `USE_BLEND=True` and widened calibration bounds for 'c' to (-0.20, 0.20).
6. Updated Pseudo-Label priority chain (v49 -> v48 -> v45 -> v42 -> v30).
"""
...
USE_BLEND = True  # Enabled for v52
...
print("=" * 75)
print("  ML OPSIDIAN v52 - COMPOUND INTERACTION & ANALYTICAL FUSION PIPELINE")
print("=" * 75)
...
# -----------------------------------------------------------------
# 1.6. SEMI-SUPERVISED PSEUDO-LABELING (v52 priority)
# -----------------------------------------------------------------
...
# -----------------------------------------------------------------
# 3. FEATURE ENGINEERING (v52 Additions)
# -----------------------------------------------------------------
...
CAT_FEATURES = [
    ...
    "downstream_district_sig", "inundation_sig" # [v52] Added to cat features
]
...
TARGET_ENC_COLS = [
    ...
    "inundation_sig" # [v52] Added to target enc
]

COMPOSITE_ENC_COLS = ["downstream_district_sig"] # [v52] High smoothing for this one
STD_ENC_COLS = [
    "district", "downstream_sig", "downstream_district_sig" # [v52] Added to std
]

IGNORE_COLS = DROP_COLS + [
    TARGET, "flood_occurrence_yes", "downstream_sig", "downstream_quad_sig", "infra_deficit_sig",
    "downstream_district_sig", "inundation_sig" # [v52] Raw string dropped for models, encoded forms used
]
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - SINGLE-SEED v52 PIPELINE")
print("=" * 75)
...
def fit_metric_stacker(X_meta, y_true, alpha=0.1):
    ...
    # dispersion stats (next 2 features) and analytical median (1 feature) and intercept are unconstrained.
...
for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
    ...
    loo_target_encode(tr_rows, va_rows, test_df, 'downstream_district_sig', TARGET, GLOBAL_MEDIAN, smoothing=SMOOTHING_COMPOSITE) # [v52] Added LOO
    loo_features = ['downstream_sig_loo_enc', 'district_loo_enc', 'downstream_district_sig_loo_enc']

    # Combine all features (deduplicated to prevent duplicate columns in XGBoost)
    ...
    # 2. CAT-MAE-1 (d5) [v52] added max_ctr_complexity=2
    ...
    # 3. CAT-MAE-2 (d5) [v52] added max_ctr_complexity=2
    ...
    # 4. CAT-RMSE (d5) [v52] added max_ctr_complexity=2
    ...
    # 7. Quantile Ensemble Base Model [v52] added max_ctr_complexity=2
    quantile_preds_va = []
    quantile_preds_te = []
    ...
    # 8. CAT-RankGauss (d5) - target transformed CatBoost model [v52] added max_ctr_complexity=2
...
# Construct Stacking Meta-Feature Matrices [v52] Added analytical median
oof_meta = np.column_stack([
    *[oof_preds[m][real_mask] for m in MODEL_NAMES],
    oof_iqr[real_mask],
    oof_skew[real_mask],
    oof_analytical[real_mask] # [v52] 11th meta-feature
])
...
tst_meta = np.column_stack([
    *[tst_preds[m] for m in MODEL_NAMES],
    tst_iqr,
    tst_skew,
    tst_analytical_fold # [v52] 11th meta-feature
])
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v52 - Raw Stacking)")
print("=" * 75)
...
fold_report.to_csv("fold_report_v52.csv", index=False)
fold_report.to_csv("submissions/fold_report_v52.csv", index=False)
...
submission.to_csv("submissions/submission_v52.csv", index=False)
print(f"[DONE] Saved submission_v52.csv ({len(submission)} rows)")

np.save("oof_v52.npy", oof_stacked)
np.save("submissions/oof_v52.npy", oof_stacked)
print(f"[DONE] Saved oof_v52.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED FALLBACK-ENABLED PER-GROUP POWER TRANSFORMATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v52)")
print("=" * 75)
...
bounds = [(0.5, 1.5), (0.5, 2.0), (-0.20, 0.20)] # [v52] Widened c bounds
...
for grp in unique_groups:
    ...
    res_grp = minimize(group_loss, x0=[a_glob, b_glob, c_glob],
                       bounds=[(0.5, 1.5), (0.5, 2.0), (-0.20, 0.20)], # [v52] Widened bounds
                       method='L-BFGS-B')
...
if USE_BLEND:
    print("\n" + "=" * 75)
    print("  DYNAMIC ANALYTICAL GROUP MEDIAN BLEND (v52)")
    print("=" * 75)
...
np.save("oof_v52_optimized.npy", final_oof)
np.save("submissions/oof_v52_optimized.npy", final_oof)
print(f"[DONE] Saved oof_v52_optimized.npy")
...
submission_opt.to_csv("submission_v52_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v52_optimized.csv", index=False)
print(f"[DONE] Saved submission_v52_optimized.csv ({len(submission_opt)} rows)")
