"""
ML Opsidian: Genesis v53 - Stacking with Target RankGauss & Quantile Dispersion
==============================================================================
Upgrades and Bug Fixes from v51:
1. Leak-Free Target RankGauss Transformer:
   - Fits a QuantileTransformer(output_distribution='normal') on the training fold's real target values.
   - Transforms targets before training a CatBoost RMSE regressor ('CAT-RankGauss').
   - Inverse-transforms out-of-fold and test predictions back to raw [0.0, 1.0] scale.
   - Adds this as the 8th base model to create target-representation diversity.
2. Quantile Dispersion Meta-Features:
   - Computes Inter-Quantile Range (IQR) and Skewness of 5-quantile predictions per fold.
   - Feeds IQR and Skewness as second-order features directly to the Level-2 Ridge Stacker.
3. Metric-Aware Constrained Stacking:
   - Bounds the 8 base prediction weights to be non-negative.
   - Keeps the 2 dispersion weights and intercept unconstrained.
"""
...
print("=" * 75)
print("  ML OPSIDIAN v53 - RANKGAUSS & QUANTILE DISPERSION STACKING")
print("=" * 75)
...
if USE_PSEUDO:
    # Use RAW un-calibrated v42 predictions to prevent double-calibration of test coordinates
    pseudo_path = "submission_v51_optimized.csv"
    if not os.path.exists(pseudo_path):
        pseudo_path = "submissions/submission_v51_optimized.csv"
    ...
    if os.path.exists(pseudo_path):
        ...
        print("   [WARNING] submission_v51_optimized.csv not found. Falling back to v30...")
        fallback_path = "submissions/submission_v51.csv"
...
print("\n" + "=" * 75)
print(f"  5-FOLD SPATIAL GROUP CV - SINGLE-SEED v53 PIPELINE")
print("=" * 75)
...
def fit_metric_stacker(X_meta, y_true, alpha=0.1):
    ...
    # dispersion stats (next 2 features) and intercept are unconstrained.
    n_base_models = len(MODEL_NAMES)
    n_dispersion = n_meta_features - n_base_models
    
    bounds = (
        [(0.0, None) for _ in range(n_base_models)] + 
        [(None, None) for _ in range(n_dispersion)] + 
        [(None, None)]
    )
...
for fold, (tr_idx, va_idx) in enumerate(gkf.split(train_df, y, groups)):
    ...
    for alpha in QUANTILE_ALPHAS:
        qmodel = cb.CatBoostRegressor(
            ...
            task_type=CB_TASK_TYPE, thread_count=-1, random_seed=SEED, verbose=False
        )
        qmodel.fit(X_tr_cat, y_tr, sample_weight=w_tr, cat_features=cat_cols, eval_set=(X_va_cat, y_va), early_stopping_rounds=50, verbose=False)
    ...
    # 8. CAT-RankGauss (d5) - target transformed CatBoost model
    ...
    cat_rg = cb.CatBoostRegressor(
        ...
        bagging_temperature=0.7, random_strength=2.0, border_count=254,
        loss_function="RMSE", eval_metric="RMSE", task_type=CB_TASK_TYPE,
        random_seed=SEED + 4, verbose=False
    )
    ...
    # Inverse-transform validation & test predictions back to original risk range
    ...
    print(f"      Fold {fold+1}/5 | XGB1_it={xgb_m1.best_iteration}  CAT1_it={cat_m1.best_iteration_}  LGB_it={lgb_m1.best_iteration_}  [{time.time()-t0:.0f}s]")
...
print("\n" + "=" * 75)
print("  GLOBAL OOF RESULTS (v53 - Raw Stacking)")
print("=" * 75)
...
fold_report.to_csv("fold_report_v53.csv", index=False)
fold_report.to_csv("submissions/fold_report_v53.csv", index=False)
...
submission.to_csv("submissions/submission_v53.csv", index=False)
print(f"[DONE] Saved submission_v53.csv ({len(submission)} rows)")

np.save("oof_v53.npy", oof_stacked)
np.save("submissions/oof_v53.npy", oof_stacked)
print(f"[DONE] Saved oof_v53.npy")

# -----------------------------------------------------------------
# 8. INTEGRATED FALLBACK-ENABLED PER-GROUP POWER TRANSFORMATION
# -----------------------------------------------------------------
print("\n" + "=" * 75)
print("  POST-HOC POWER TRANSFORMATION OPTIMIZATION (v53)")
print("=" * 75)
...
if USE_BLEND:
    print("\n" + "=" * 75)
    print("  DYNAMIC ANALYTICAL GROUP MEDIAN BLEND (v53)")
    print("=" * 75)
    
    oof_analytical_clean = oof_analytical[real_mask]
    
    # We will optimize BLEND_ALPHA targeting the competition metric on OOF
...
np.save("oof_v53_optimized.npy", final_oof)
np.save("submissions/oof_v53_optimized.npy", final_oof)
print(f"[DONE] Saved oof_v53_optimized.npy")
...
submission_opt.to_csv("submission_v53_optimized.csv", index=False)
submission_opt.to_csv("submissions/submission_v53_optimized.csv", index=False)
print(f"[DONE] Saved submission_v53_optimized.csv ({len(submission_opt)} rows)")
