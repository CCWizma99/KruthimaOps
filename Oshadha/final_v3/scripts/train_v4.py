...
Key upgrades from v2:
  - StratifiedKFold on 10-bin target (stable distribution per fold)
  - Learning rate 0.02 -> 0.05 (faster exploration before early stop)
  - Early stop patience 50 -> 100 (more room to find interactions)
  - KFold-safe target encoding for district + lat/lon spatial grid
  - 6 new features (16 total engineered, 58 total fed to models)
  - Per-model X format (XGB=int codes, LGB=Categorical, CAT=string)
  - Inverse-RMSE weighted ensemble (better models get more weight)
  - Isotonic regression calibration (fixes prediction range collapse)

XGBoost 3.x + pandas 3.x compatibility notes:
  - early_stopping_rounds must be in constructor, not fit()
  - String pd.Categorical causes Arrow columnar dtype error -> use int codes instead
  - enable_categorical=False for XGBoost, pass .cat.codes as int32 columns
...
def to_xgb_fmt(df):
    ...
    Covers both named cat_feature_names AND any column that inherits Categorical
    dtype (e.g. target encoding columns created by .map() on a Categorical)."""
    df = df.copy()
...
for fold, (tr_idx, va_idx) in enumerate(skf.split(train_df, y_bins)):
    ...
    # .astype(str) before map + explicit float cast: prevents Categorical dtype
    # inheritance when mapping on a pd.Categorical column (pandas 3.x behaviour)
    ...
    # We add district_target_enc and grid_target_enc too, since they're powerful
    poly_feats_with_enc = poly_feats + ["district_target_enc", "grid_target_enc"]
    
    from sklearn.preprocessing import PolynomialFeatures
    poly = PolynomialFeatures(degree=2, interaction_only=True, include_bias=False)
    
    # Fit poly only on tr_rows
    ...
    # We only want the new interactions, so skip the first len(poly_feats_with_enc) which are just the original features
    ...
    test_df_fold = pd.concat([test_df, te_poly_df], axis=1) # Need fold-specific test_df for poly features
    ...
    xgb_model = xgb.XGBRegressor(
        ...
        learning_rate         = 0.05,
        max_depth             = 7,
        min_child_weight      = 1,   # lowered to allow more specific splits
        subsample             = 0.8,
    ...
    lgb_model = lgb.LGBMRegressor(
        objective          = "regression_l1",  # <--- Changed to MAE
        n_estimators       = 3000,
        learning_rate      = 0.05,
        num_leaves         = 127,
        max_depth          = -1,
        min_child_samples  = 10,   # lowered
        subsample          = 0.8,
        subsample_freq     = 1,
        colsample_bytree   = 0.75,
        reg_alpha          = 0.1,
    ...
    cat_model = cb.CatBoostRegressor(
        ...
        random_strength       = 1,
        border_count          = 128,
        loss_function         = "MAE",   # <--- Changed to MAE
        eval_metric           = "MAE",
        task_type             = "CPU",
        random_seed           = 42,
        verbose               = False
